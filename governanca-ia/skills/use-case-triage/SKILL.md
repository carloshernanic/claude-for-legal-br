...
description: >
  Classifica caso de uso de IA proposto contra o registro — aprovado, condicional
  ou não aprovado — e produz condições obrigatórias e próximos passos. Sinaliza
  handoffs entre plugins (privacy/RIPD ou product counsel). Aciona quando o(a)
  usuário(a) diz "tria esse caso de uso", "podemos usar IA para X", "isso está
  aprovado", "o que precisamos fazer para usar IA em X".
argument-hint: "[descreva o caso de uso, ou 'lote' para triagem de lista]"
---

# /use-case-triage

> **Adaptação BR não oficial.** Triagem aplica registro e linhas vermelhas
> configurados no CLAUDE.md, calibrado para LGPD (especialmente art. 20 — decisão
> automatizada; art. 38 — RIPD), Resoluções CD/ANPD, Marco Civil, CDC,
> Resoluções CNJ 332/2020 e 615/2025 (Judiciário), Resoluções TSE (eleitoral),
> Provimento CFM 2.314/2022 (saúde), Lei 14.811/2024 (deepfake/CSAM), CF art.
> 5º, Lei 7.716 e Lei 12.288 (não-discriminação). PL 2338/2023 `[em tramitação
> — verificar]`. EU AI Act como benchmark / obrigatório se houver nexo na UE.
> US (NYC LL144, CO AI Act, IL BIPA) como comparativo.

1. Read `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`. Confirm registry is populated — if not, stop and direct to setup.
...
```
/governanca-ia:use-case-triage "Sales team wants to score leads with AI automatically"
```
...
Before triaging, always read `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`. The use case registry and red lines there
...
>
> **Two choices:**
> - Run `/governanca-ia:cold-start-interview` (2 minutes) to configure your profile, then I'll triage tailored to YOUR practice.
...
Check the use case registry in `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` for a direct or close match.
...
Depois cheque o caso de uso contra TODO regime no `## Footprint regulatório` do perfil, não só o primário. Sinalize conflitos:
- "APROVADO pelo registro padrão, mas LGPD art. 20 exige direito à revisão se a decisão automatizada afetar titulares — confirme se há decisões consequenciais sem revisão humana."
- "Tier padrão pelo framework de governança, mas aciona RIPD (LGPD art. 38) se houver alto risco aos titulares; Resolução CD/ANPD aplicável."
- "Aprovado para uso interno, mas se afetar consumidores aciona CDC arts. 6º III, 31 (informação) e potencialmente art. 39 IV (vulnerabilidade)."
- "Aprovado fora do Judiciário, mas Resolução CNJ 332/2020 e 615/2025 trazem requisitos próprios se uso for em órgão judicial."
- "Aprovado para uso geral, mas em ano eleitoral Resoluções TSE exigem rotulagem de conteúdo sintético."
- "APROVADO sob lei brasileira, mas aciona EU AI Act Art. 27 FRIA se residentes na UE forem afetados — confirme."
...
**Governance tier:** [Standard / Elevated / High — per `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`]
...
`/governanca-ia:aia-generation [use case]`
...
**Handoff de privacidade:** Se o caso envolve dados pessoais — dados de empregado(a), de cliente, comportamentais — sinalize:

> "Este caso envolve tratamento de dados pessoais. Provavelmente requer RIPD (LGPD art. 38) além da AIA. Use `/privacidade:pia-generation [caso de uso]`, se o plugin estiver instalado, para rodar em paralelo. Considere também: base legal (LGPD arts. 7º e 11), informação ao titular (art. 9º), direito à revisão se houver decisão automatizada (art. 20), transferência internacional se aplicável (arts. 33-36; Resolução CD/ANPD 19/2024)."
...
> Use `/produto:launch-review`, if the plugin is installed — it will detect
...
> "Add to `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` → Use case registry. This means next time the same request
