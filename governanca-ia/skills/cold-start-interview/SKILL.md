...
description: >
  Roda a entrevista de cold-start — aprende a prática de governança de IA e
  escreve `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`
  a partir da política de IA da empresa, uma avaliação de impacto de referência
  e contratos-chave de fornecedor de IA. Aciona quando o perfil estiver ausente
  ou contiver marcadores `[PLACEHOLDER]`, ou quando o(a) usuário(a) disser
  "configure o plugin de governança de IA", "onboard", "configura governança de IA".
argument-hint: "[--redo | --check-integrations]"
---

# /cold-start-interview

> **Adaptação BR não oficial.** Entrevista calibrada para o framework brasileiro:
> LGPD (Lei 13.709/2018), PL 2338/2023 (Marco Legal da IA — `[em tramitação —
> verificar]`), Resoluções CD/ANPD (4/2023 sandbox, 15/2024 incidentes, 19/2024
> transferência internacional), Marco Civil (Lei 12.965/2014), CDC, Resoluções
> CNJ 332/2020 e 615/2025 (IA no Judiciário), Resoluções TSE (IA em campanhas),
> Provimento CFM 2.314/2022 (telemedicina), Lei 14.811/2024 (CSAM/deepfake), PL
> 2630, princípios OCDE de IA, e ainda EU AI Act como benchmark global,
> NIST AI RMF como framework técnico, e US (EO 14110, CO AI Act, NYC LL144, IL
> BIPA) como referência comparativa.

1. Check `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` — if populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.claude/plugins/cache/claude-for-legal/governanca-ia/*/CLAUDE.md` but not at the config path, copy it to the config path and tell the user what was migrated.
6. Write `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` (create parent directories as needed). Show summary. Offer first task.

## Flags

- `--redo` — re-run the full interview and overwrite `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` without re-running the full interview. Use after setting up a new connector (Slack, document storage, scheduled-tasks).
...
/governanca-ia:cold-start-interview --check-integrations
```
...
Read `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`:
...
If a CLAUDE.md exists at the old cache path `~/.claude/plugins/cache/claude-for-legal/governanca-ia/*/CLAUDE.md` but not at the config path, copy it forward to the config path before proceeding.
...
> **`governanca-ia` is for people who run AI governance: use-case triage, impact assessments, vendor AI review, policy monitoring.** Not your area? `/builder-hub:related-skills-surfacer`.
>
...
> "This plugin maintains: your practice profile (governance tiers, red lines, policy commitments), a use-case registry, impact assessments, and vendor AI reviews — all in `~/.claude/plugins/config/claude-for-legal/governanca-ia/`. It learns how you actually work — your practice, your risk calibration, your house conventions — and writes that into a plain-text file the plugin reads from every time. Everything you answer can be changed later."
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/governanca-ia:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run /governanca-ia:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
You don't need all of these. Core features work with file access alone. If you set something up later, re-run `/governanca-ia:cold-start-interview --check-integrations`.
...
> add the rest later with `/governanca-ia:ai-inventory add`. Or skip
...
with `/governanca-ia:ai-inventory add` later?" If they accept, run the
Add flow and the classification walk-through from
`governanca-ia/skills/ai-inventory/SKILL.md` for each system. Save to
...
### Parte 2: Footprint regulatório (2-3 min)

> Quais regulações estão de fato no seu radar? Não quero assumir — me diga o que é real para você. (Isso alimenta /reg-gap-analysis e /policy-monitor — a gap analysis diffa novas regulações contra o escopo declarado, e o policy-monitor só observa regimes marcados em escopo.)

**Não assuma que qualquer regulação se aplica. Pergunte ao(à) usuário(a) quais regimes acha que se aplicam, depois pesquise as regulações de IA atualmente em vigor ou em tramitação nas jurisdições onde a empresa opera, implanta IA ou tem partes afetadas. Esse cenário muda rápido — verifique a atualidade.**

Prompts para conduzir:

- **Footprint jurisdicional** — onde estão clientes, empregados(as), titulares de dados e operações? A IA toca pessoas em algum desses lugares?
- **Regimes BR centrais** — LGPD (em especial art. 20 — decisão automatizada; art. 38 — RIPD; art. 48 — incidente); Resoluções CD/ANPD (4/2023, 15/2024, 19/2024); Marco Civil (Lei 12.965/2014; art. 11; Tema 533 STF; RE 1.037.396); CDC (arts. 12, 14, 37, 39); CF (art. 5º — privacidade, isonomia, dignidade); Lei 7.716 e Lei 12.288 (não-discriminação); Lei 9.610/98 (autoral — debate sobre treinamento, PL 2338); Lei 14.811/2024 (deepfake/CSAM); PL 2630 (em tramitação).
- **PL 2338/2023 (Marco Legal da IA — em tramitação)** — pergunte se a empresa está acompanhando; trate como benchmark e referência prospectiva, não como lei vigente. Tag `[em tramitação — verificar]`.
- **Setoriais BR** — Resolução CNJ 332/2020 e 615/2025 (IA no Judiciário); Resoluções TSE (IA em campanhas, em ano eleitoral); Provimento CFM 2.314/2022 (telemedicina, por analogia para saúde); BCB 4.865 e CVM 29 (sandbox financeiro); deliberações BCB/CVM sobre IA em serviços financeiros; CLT + jurisprudência TST sobre monitoramento de empregados(as); ECA + Lei 14.811/2024 para infantojuvenil.
- **Cross-border** — se a empresa tem usuários, clientes ou empregados fora do BR (especialmente UE), avaliar EU AI Act como obrigatório, não só benchmark; GDPR; US sectoral.
- **Comparativos US** (mencionar como benchmark, não como lei aplicável a operação BR pura): EO 14110 revogada/substituída — `[verificar]`; Colorado AI Act; NYC Local Law 144 (AEDT); IL BIPA (biometria); state AI laws.
- **Compromissos contratuais** — clientes corporativos exigem disclosures de IA, AIAs/RIPDs, cláusulas específicas de IA?
...
Categorias comuns a sondar se travarem: dados biométricos (LGPD art. 11; CF art. 5º), detecção de emoção, inferência política/religiosa/origem (LGPD art. 11), decisões adversas totalmente automatizadas afetando emprego ou crédito (LGPD art. 20 — direito à revisão), usos envolvendo crianças e adolescentes (ECA; LGPD art. 14; Lei 14.811/2024), conteúdo sintético/deepfake (Lei 14.811/2024; Resoluções TSE), saúde (Provimento CFM 2.314/2022), eleitoral (Resoluções TSE), IA em Judiciário sem aderência à Resolução CNJ 332/2020 e 615/2025.
...
**Footprint regulatório:** [Liste apenas o que efetivamente se aplica — LGPD (em especial art. 20, 38, 48); Resoluções CD/ANPD; Marco Civil; CDC; PL 2338/2023 (em tramitação); setoriais (CNJ 332/2020 e 615/2025; TSE em ano eleitoral; Provimento CFM 2.314/2022; deliberações BCB/CVM); Lei 14.811/2024 (deepfake/CSAM); PL 2630; EU AI Act (se houver nexo na UE) / Colorado AI Act / NYC LL144 / BIPA apenas como benchmark ou se aplicável por nexo / requisitos contratuais]
...
>
> - **Review vendor AI terms** — e.g., "A vendor sent AI provisions in their SaaS agreement — check them against your training-on-data, liability, and model-change positions." Try: `/governanca-ia:vendor-ai-review`
> - **Triage a proposed AI use case** — e.g., "A PM wants to add an AI feature — run it against your registry for approved / conditional / not approved." Try: `/governanca-ia:use-case-triage`
> - **Run an AI impact assessment** — e.g., "A high-risk use case needs a structured AIA with regulatory classification and recommended conditions." Try: `/governanca-ia:aia-generation`
> - **Diff a new AI regulation against your posture** — e.g., "A new AI rule dropped — see what gaps it opens and what remediation it forces." Try: `/governanca-ia:reg-gap-analysis`
> - **Sweep for policy drift** — e.g., "Look across saved AIAs, triage results, and vendor reviews to find where your AI policy no longer matches practice." Try: `/governanca-ia:policy-monitor`
>
...
5. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your configuration is at `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
   > - Edit the file directly for a quick change
   > - Run `/governanca-ia:cold-start-interview --redo` for a full re-interview
   > - Run `/governanca-ia:cold-start-interview --check-integrations` to re-check what's connected
   >
