...
description: >
  Diffa uma nova regulação ou orientação de IA contra a postura atual de
  governança — explicita gaps, prioridades e plano de remediação com donos(as)
  e prazos. Aciona quando uma regulação de IA se move (ou você descobre uma que
  perdeu), ou quando o(a) usuário(a) diz "saiu regulação nova", "[regulação] nos
  afeta?", "gap analysis para PL 2338 / Resolução ANPD / EU AI Act", "check de
  compliance contra [lei ou orientação]", ou cola texto regulatório.
argument-hint: "[nome da regulação, ou cole texto regulatório, ou anexe documento]"
---

# /reg-gap-analysis

> **Adaptação BR não oficial.** Gap analysis prioriza regimes BR: LGPD (especialmente
> arts. 5º, 6º, 7º, 11, 18, 20, 38, 48), Resoluções CD/ANPD (4/2023, 15/2024,
> 19/2024), Marco Civil (Lei 12.965/2014; Tema 533 STF; RE 1.037.396), CDC (12,
> 14, 37, 39), CF (art. 5º), Lei 7.716 e Lei 12.288, Lei 14.811/2024, Resoluções
> CNJ 332/2020 e 615/2025, Resoluções TSE em ano eleitoral, Provimento CFM
> 2.314/2022, deliberações BCB/CVM, sandbox (Resolução BCB 4.865, Resolução CVM
> 29, Resolução CD/ANPD 4/2023). PL 2338/2023 e PL 2630 — `[em tramitação —
> verificar]`. EU AI Act como benchmark global / obrigatório se houver nexo na
> UE. NIST AI RMF como framework técnico. US (EO 14110, CO AI Act, NYC LL144,
> IL BIPA) como referência comparativa.

1. Read `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`. Confirm regulatory footprint and use case registry are populated.
...
4. Extract requirements. Diff against current state in `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`.
...
```
/governanca-ia:reg-gap-analysis "EU AI Act high-risk provisions"
```
...
O PL 2338/2023 avança em comissão. A ANPD publica nova Resolução CD/ANPD sobre
IA, incidentes ou transferência internacional. O CNJ atualiza a Resolução
332/2020 (e agora a 615/2025) sobre IA no Judiciário. O TSE edita Resolução
sobre IA em campanha em ano eleitoral. O BCB delibera sobre IA em serviços
financeiros. STF/STJ firma tese (p.ex., Tema 533, RE 1.037.396). Algo se move
— e agora você precisa saber o que, se algo, precisa mudar. EU AI Act, NIST AI
RMF e leis estaduais US permanecem relevantes para empresas com nexo
internacional.

This skill diffs the new requirement against your current AI governance posture
(per `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` — use case registry, vendor positions, impact assessment practices,
...
**We currently:** [what `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` / AI policy / use case registry / assessment
practice shows]
...
**To regulatorio plugin, if the plugin is installed:** This skill is the manual
...
- It doesn't track regulatory changes proactively. It runs when you point it at a
  change. For proactive monitoring, see the `regulatorio` plugin, if the plugin is installed.
