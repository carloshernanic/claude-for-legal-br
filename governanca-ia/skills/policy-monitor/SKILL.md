...
name: policy-monitor
description: >
  Mantém a política de IA atual com a prática — varredura semanal de AIAs/RIPDs
  salvos, resultados de triagem e revisões de fornecedor para encontrar drift de
  política, ou consulta direta para nova prática de IA proposta. Aciona quando
  o(a) usuário(a) diz "varredura de política", "nossa política de IA cobre
  isso", "queremos começar a fazer X — a política precisa de atualização",
  "rode o policy-monitor", ou em agenda recorrente.
argument-hint: "[descreva uma nova prática de IA proposta — ou omita / use --sweep para modo crawl]"
---

# /policy-monitor

> **Adaptação BR não oficial.** A política de IA precisa refletir LGPD (em
> especial art. 20 — decisão automatizada), Resoluções CD/ANPD, Marco Civil,
> CDC, Resoluções CNJ 332/2020 e 615/2025 (se Judiciário), Resoluções TSE (em
> ano eleitoral), Provimento CFM 2.314/2022 (se saúde), Lei 14.811/2024
> (deepfake), e o status atual do PL 2338/2023 `[em tramitação — verificar]`.
> Atenção especial a drift quando: AIAs aprovaram decisões automatizadas (LGPD
> art. 20), novos fornecedores tratam dados pessoais com transferência
> internacional (Resolução CD/ANPD 19/2024), ou houve incidente com obrigação
> de comunicação à ANPD (LGPD art. 48; Resolução CD/ANPD 15/2024).

**Sweep mode** (no argument or `--sweep`):
1. Read `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` → outputs folder path, AI policy document, last sweep date.
...
6. Flag any use cases in outputs not yet added to the `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` registry.
7. Present results to the human. Only after acknowledgment, update `Last policy sweep` and `gaps_found` in `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`.

**Direct query mode** (with description argument):
1. Read `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` → current policy commitments, use case registry, actual policy document.
...
**Recurring runs:**
Set up a recurring reminder in your own scheduler to run `/governanca-ia:policy-monitor` weekly. Scheduled execution requires a scheduled-tasks integration, which is not bundled with this plugin.

```
/governanca-ia:policy-monitor
/governanca-ia:policy-monitor "We want to use AI to automatically flag expense reports for review"
```
...
> enable the crawl sweep, run `/governanca-ia:cold-start-interview` and provide the outputs
> folder path."

Read the actual AI or acceptable use policy document from the path in `## Outputs`
→ **AI policy document**. The commitments in `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` are a summary; the actual
...
**Do not update `Last policy sweep` or `gaps_found` automatically.** After the sweep results are presented, wait for the human to acknowledge them ("sweep acknowledged," "results reviewed," or equivalent). Only then update `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`:
...
- Extract: vendor added, data use terms agreed to, any AI-specific provisions
  accepted that differ from standard positions in `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`
...
> Exemplo: Política de IA diz "não usamos IA em decisões trabalhistas." Uma AIA
> aprovou ferramenta de triagem de currículos assistida por IA com revisão
> humana obrigatória. Política precisa atualizar — mesmo com revisão humana, IA
> está envolvida em decisões trabalhistas. "Não usamos IA" não é mais preciso.
> (Pode acionar LGPD art. 20 — direito à revisão — e CLT/jurisprudência TST
> sobre monitoramento e seleção.)
...
> Exemplo: Política diz "usamos IA para melhorar nossos produtos e serviços."
> Uma AIA aprovou feature de IA para rascunhos de atendimento. Política
> tecnicamente cobre mas é vaga. Recomendável ser mais específico para que
> consumidores saibam com o que interagem — alinhamento com CDC arts. 6º III e
> 31 (direito à informação) e LGPD art. 9º (informação ao titular).
...
[Any use cases approved since the last sweep that aren't yet in the `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md`
...
[If this use case isn't in the registry: "Add to `~/.claude/plugins/config/claude-for-legal/governanca-ia/CLAUDE.md` → Use case registry:"]
```
...
The weekly sweep is designed to run on a recurring cadence. Set up a recurring reminder in your own scheduler to run `/governanca-ia:policy-monitor` weekly. Scheduled execution requires a scheduled-tasks integration, which is not bundled with this plugin.
