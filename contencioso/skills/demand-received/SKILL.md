...
description: Triagem de notificação extrajudicial recebida — extrai campos, cruza com a carteira, avalia mérito, apresenta opções de resposta com recomendação, e roteia para matter-intake ou demand-intake se escalonamento for cabível. Use quando o(a) usuário(a) disser "recebemos uma notificação", "triagear esta notificação", ou compartilhar notificação recebida para avaliar.
argument-hint: "[caminho-para-recebida] [--slug=slug-customizado]"
---

# /demand-received

> **Adaptação BR não oficial.** Notificações recebidas no BR: notificação extrajudicial cartorária (Lei 6.015/1973 art. 160), notificação judicial (CPC 726-729), interpelação, cessar-e-desistir, ofício de regulador (BCB/CVM/CADE/ANPD/ANVISA/ANATEL), oficiamento do MP (Lei 8.625/1993 art. 26), requisição judicial (CPC 438). Reação no BR: prazo de resposta depende do contrato / norma — não há "30-day demand response" genérico. **Notificação para constituir em mora (CC 397)** dispara prazo do próprio instrumento. **Interrompe prescrição (CC 202 II / III).** **Notificação coletiva / inquérito civil (Lei 7.347/1985)** pode preceder ACP. **Recusa pode gerar presunção (CPC 400).** Conferir LGPD se a notificação envolve titular de dados (ANPD; art. 41 §1º). Aviso ao seguro: regra contratual + CC 771 (boa-fé).

1. Read the incoming document from provided path.
2. Load `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml` for portfolio cross-check.
3. Load `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → risk calibration, landscape, demand-letter practice.
...
6. Write `~/.claude/plugins/config/claude-for-legal/contencioso/inbound/[slug]/triage.md`. Copy or link incoming to `~/.claude/plugins/config/claude-for-legal/contencioso/inbound/[slug]/incoming.[ext]`.
...
- `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml` — scan for related matters (same counterparty, overlapping counterparties via entity relationships, or matter type + recent date)
- `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → risk calibration (for merit assessment), landscape (is the sender a frequent adversary?), demand-letter practice (house tone and response defaults)
...
- **Leverage and pressure** — are they credibly prepared to sue? Do they have capacity? Are they a repeat-litigant adversary per `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`?
...
- Standalone → leave in `~/.claude/plugins/config/claude-for-legal/contencioso/inbound/`; no portfolio change.
