---
name: legal-hold
description: Emite, renova, libera ou reporta ordens de preservação documental — redige a notificação de preservação como .docx, atualiza campos de preservação no _log.yaml, e agenda a próxima renovação. Use quando o(a) usuário(a) disser "emitir preservação", "renovar preservação", "liberar preservação", ou pedir relatório de portfólio sobre status de preservação.
argument-hint: "[slug] [--issue | --refresh | --release | --status]"
---

# /legal-hold

> **Adaptação BR não oficial.** No Brasil **não há doutrina equivalente ao "litigation hold" americano** (FRCP 37(e) + spoliation sanctions). O dever de preservação documental decorre de:
>
> | Original (US) | Equivalente funcional BR |
> |---|---|
> | Litigation hold (FRCP 37(e); Zubulake; sanctions) | Boa-fé documental (CC arts. 186, 422, 927); presunção de veracidade pela recusa (CPC art. 400); exibição (CPC 396-404) |
> | Spoliation sanctions | Não há doutrina codificada; analogamente, **CPC 400** (recusa = presunção de veracidade), **CPC 80** (litigância de má-fé), **CP 305** (supressão de documento), **CP 311** (falsificação de prova) |
> | Document retention policy | Política de retenção sob LGPD (Lei 13.709/2018 arts. 15-16 — eliminação ao fim do tratamento), Marco Civil (Lei 12.965/2014 art. 13 — guarda de logs), CT (Lei 9.430/1996 art. 37 — guarda fiscal 5 anos), CLT 11 (guarda de documentos trabalhistas) |
> | Custodian (ESI) | Custodiante (titular de caixa de e-mail, drive, sistema); guarda lícita LGPD art. 6º VI |
> | Hold notice / preservation letter | **Notificação de preservação documental** — ofício interno a custodiantes; pode ser registrada em cartório (Lei 6.015/1973) para data certa |
> | Hold release | Liberação documental — quando matéria encerra (acordo, trânsito em julgado, decadência) |
> | Privilege flag (work product) | Sigilo profissional (EOAB 7º XIX) — material elaborado por/para advogado(a) |
>
> **Pontos BR críticos:**
> 1. **LGPD impõe limites à guarda.** Manter dados pessoais além do necessário viola arts. 15-16. Preservação documental deve ser **limitada ao escopo da matéria** e ter base legal (CC 422 — boa-fé; CPC — defesa em juízo, art. 7º IX LGPD).
> 2. **Recusa = presunção.** CPC 400 — se a parte recusa exibir documento sob seu poder, presume-se verdadeiro o fato. Inversa do "adverse inference" americano.
> 3. **Crime contra a administração da justiça.** Destruição dolosa de documento sob preservação pode caracterizar fraude processual (CP 347) ou supressão (CP 305).
> 4. **Marco Civil — guarda obrigatória de logs:** provedores de conexão (1 ano, art. 13) e aplicação (6 meses, art. 15) têm dever de guarda independente. Preservar logs em demanda envolve oficiar ao provedor (CPC 438; MC 22).
> 5. **CLT 11** — direito trabalhista prescreve 5 anos / 2 do término; documentos trabalhistas devem ser guardados nesse horizonte mesmo sem preservação específica.

1. If `--status` (no slug): read `_log.yaml`, produce portfolio-wide hold report.
2. Otherwise: load `~/.claude/plugins/config/claude-for-legal/contencioso/matters/[slug]/matter.md` + log row.
3. Load `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → privilege markings, hold template pointer, escalation norms.
...
- `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml` — log row (legal_hold fields + status)
- `~/.claude/plugins/config/claude-for-legal/contencioso/matters/[slug]/matter.md` — matter context (counterparty, facts, key custodians from internal_owners)
- `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` — house style for litigation hold template pointer, privilege marking, escalation norms
...
> "I don't see [matter slug] in the matter log. Run `/contencioso:matter-intake` first so the conflicts check runs and the matter workspace is set up. I won't issue, refresh, or release a legal hold on a matter that hasn't been intaken — the conflicts check is the gate, and a hold issued against an unmanaged matter has no `_log.yaml` row to track `last_refresh` / `next_refresh` / `released` against."
...
**Before issuing the hold to custodians (the consequential act):** Read `## Who's using this` in `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`. If the Role is Non-lawyer:
...
**Draft the notice** to each custodian, using the house template in `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` if one is configured; otherwise the default template below.
...
DATE: [effective date]
TO: [custodian name]
FROM: [signer — per `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` default]
...
**Writes:**
- `~/.claude/plugins/config/claude-for-legal/contencioso/matters/[slug]/legal-hold-v1.docx` via the `docx` skill
- Appends to `~/.claude/plugins/config/claude-for-legal/contencioso/matters/[slug]/history.md`:
  ```
...
**Writes:**
- `~/.claude/plugins/config/claude-for-legal/contencioso/matters/[slug]/legal-hold-v[N].docx` (next version number)
- `history.md` entry
...
**Before releasing the hold (the consequential act — preservation obligations resume normal retention):** Read `## Who's using this` in `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`. If the Role is Non-lawyer:
...
**Writes:**
- `~/.claude/plugins/config/claude-for-legal/contencioso/matters/[slug]/legal-hold-release.docx`
- `history.md` entry
