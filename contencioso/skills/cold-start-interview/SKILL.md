...
description: Cold-start do plugin de contencioso — ramifica por papel (in-house, advogado(a) de escritório, solo) e por polo (autor, réu, ambos), captura calibração de risco, panorama e estilo da casa, e grava o perfil prático em CLAUDE.md. Use em instalação nova, quando o(a) usuário(a) quiser configurar ou refazer o perfil prático, ou para reconferir integrações disponíveis.
argument-hint: "[--redo | --check-integrations]"
---

# /cold-start-interview

> **Adaptação BR não oficial.** A configuração do perfil prático foi recalibrada para realidades brasileiras: papéis (in-house BR vs. escritório vs. solo); polos (autor / réu / ambos); calibração de risco usa provisão contábil (CPC 25), Fato Relevante (Resolução CVM 80), divulgação CVM, escalonamento ao GC; foros frequentes são JT, JEC, Justiça Estadual (varas empresariais, fazenda pública), TRFs, TRTs, TST, STJ, STF, e câmaras arbitrais (CAM-CCBC, CAM B3, CBMA, FGV); seguros incluem D&O, RC Trabalhista, Cyber e RC Geral; cobertura de "discovery" é substituída por exibição (CPC 396-404), produção antecipada (CPC 381), requisição (CPC 438). Substituir conectores PACER/CourtListener/Trellis por PJe / e-SAJ / e-Proc / Projudi / DataJud / JusBrasil / Lexml.

1. Check `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`. If already populated and no `--redo`, ask before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.claude/plugins/cache/claude-for-legal/contencioso/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`. Date the footer.
7. Confirm with the user before finalizing: "Here's what I captured — anything wrong?"

## Flags

- `--redo` — re-run the full interview and overwrite `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` without re-running the full interview. Use after setting up a new connector (DMS, document storage, Gmail, scheduled-tasks, CLM).
...
> **`contencioso` is for people who work litigation — managing a portfolio of matters in-house, drafting briefs and doing discovery at a firm, or both as a solo practitioner.** Not your area? `/builder-hub:related-skills-surfacer`.
>
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/contencioso:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration with a `<!-- SETUP PAUSED AT: [section name] — run /contencioso:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
- **Demand-letter practice** — *not asked here.* Demand posture (tone, time limits, marking, signer) is set per matter, not per practice — the solo equivalent of "who signs" answers itself (you), and tone/marking/timing depend on the specific dispute. `/contencioso:demand-intake` will ask when it drafts.
...
>
> Want to fill any of these now, or leave them as placeholders? You can also fill them later via `/contencioso:cold-start-interview --redo` or by editing the plugin config directly. This one is worth thinking about before I write: [name the most important gap and why].
...
>
> - **Intake a new matter** — e.g., "Uniform intake questions, writes matter.md + history.md, appends to the portfolio log." Try: `/contencioso:matter-intake`
> - **Triage an inbound demand** — e.g., "Options analysis, portfolio cross-check, handoff to matter intake if it graduates." Try: `/contencioso:demand-received`
> - **Draft a demand letter** — e.g., "Privilege / FRE 408 gate, .docx output, post-send checklist, matter-creation offer." Try: `/contencioso:demand-draft`
> - **Build a deposition outline** — e.g., "Docs + topics + impeachment + exhibits, tied to case theory." Try: `/contencioso:deposition-prep`
> - **Issue or refresh a legal hold** — e.g., "Draft the hold memo, update the log, schedule a refresh." Try: `/contencioso:legal-hold`
> - **Portfolio rollup** — e.g., "Risk distribution, upcoming deadlines, stale matters across the active portfolio." Try: `/contencioso:portfolio-status`
>
...
- If `in-house`: "The in-house practice profile is now written. Every matter intake will read from it. Want to run `/contencioso:matter-intake` on your most live matter to see it in action?"
...
- If `solo`: "Your solo practice profile is written — caseload shape, fee economics, how you run the office — plus the case-theory and brief-style work for a live matter. Want to run `/contencioso:matter-intake` on your most live matter and see what the intake looks like with your configuration?"

### Close with the "you can change anything later" note

> "Your practice profile is at `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
> - Edit the file directly for a quick change
> - Run `/contencioso:cold-start-interview --redo` for a full re-interview
> - Run `/contencioso:cold-start-interview --new-matter` to reuse the practice profile on a new matter (firm-associate / solo)
> - Run `/contencioso:cold-start-interview --check-integrations` to re-check what's connected
>
