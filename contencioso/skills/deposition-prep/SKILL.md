...
description: Monta roteiro de oitiva / depoimento pessoal para audiência — puxa documentos da testemunha ou parte dos arquivos da matéria, organiza tópicos em torno da teoria do caso, e levanta material para contradita. Use quando o(a) usuário(a) disser "preparar oitiva de [testemunha]", "rol de perguntas para [nome]", "preparar depoimento pessoal de [parte]".
argument-hint: "[nome da testemunha ou parte]"
---

# /deposition-prep

> **Adaptação BR não oficial — ruptura profunda com "deposition" americana.**
>
> No Brasil **NÃO existe deposition no sentido US (interrogatório pré-julgamento conduzido pelo(a) advogado(a) fora do tribunal, com transcrição taquigráfica).** O que existe:
>
> | Original (US) | Equivalente funcional BR |
> |---|---|
> | Deposition (testemunha) | **Oitiva de testemunha** (CPC arts. 442-463) — em audiência, perante juiz |
> | Deposition (parte) | **Depoimento pessoal** (CPC arts. 385-388) — em audiência, perante juiz; pena de confissão pelo silêncio |
> | Examination by attorney | **Sistema presidencialista** (CPC art. 459) — perguntas são feitas ao juiz, que pode reformular, indeferir, ou autorizar. Cross-examination direta NÃO existe |
> | Deposition transcript | **Termo de audiência** + gravação audiovisual (CPC art. 367 §5º; Resolução CNJ 105/2010) |
> | Document subpoena | **Ofício** (CPC 438) ou **exibição** (CPC 396-404) |
> | Privilege objection | Recusa fundada em **sigilo profissional** (EOAB 7º XIX; CPC 388 II; CPP 207) ou **segredo de justiça** (CPC 189) |
> | Impeachment | **Contradita** (CPC 457 §1º) — antes do depoimento, alegar suspeição/impedimento; ou **acareação** (CPC 461) |
> | Adverse witness | **Testemunha suspeita / impedida** (CPC 447 §§ 2º-4º) — depõe como informante |
> | Errata sheet | Ratificação em termo de audiência (CPC 460) |
>
> **Diferenças críticas:**
> 1. **Audiência única.** A oitiva ocorre na **audiência de instrução** (CPC 357 V; 358), em juízo. Não há "depo" fora do tribunal salvo prova emprestada / produção antecipada (CPC 381).
> 2. **Perguntas via juiz.** O(a) advogado(a) **não pergunta diretamente** à testemunha — formula a pergunta ao juízo, que repassa, reformula ou indefere. CPC 459 alterado em 2015 permitiu o(a) advogado(a) perguntar diretamente desde que sob controle do(a) juiz(a), mas pode ser controlado.
> 3. **Compromisso.** Testemunha presta compromisso (CPC 458); falso testemunho é crime (CP 342).
> 4. **Parte não testemunha em causa própria** — depõe como **parte** (depoimento pessoal CPC 385), pena de **confissão** pelo silêncio ou pelas evasivas. Diferente de testemunha sob compromisso.
> 5. **Sigilo profissional não-renunciável pelo(a) advogado(a)** (EOAB 7º XIX) — testemunha advogado(a) pode se recusar quanto a fatos sob sigilo.
>
> **Substituir referências a:** Everlaw / Relativity / DISCO → busca em pasta da matéria (Drive / SharePoint / GED); PJe / e-SAJ / e-Proc para peças e autos; JusBrasil para julgados sobre a testemunha (perfil de testemunha técnica recorrente). FRE → CPC arts. 369-484. FRCP 30/45 → CPC 442-463 (testemunha) e 385-388 (parte).

1. Load `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → case theory, key facts.
...
`~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → case theory (theory, pivot fact, key facts for/against), eDiscovery platform.

**Conflicts gate — unbypassable.** Before building an outline, check `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:

> "I don't see [matter slug] in the matter log. Run `/contencioso:matter-intake` first so the conflicts check runs and the matter workspace is set up. I won't build a deposition outline on a matter that hasn't been intaken — the conflicts check is the gate."
...
- Facts from `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → key facts for us, that this witness can establish
