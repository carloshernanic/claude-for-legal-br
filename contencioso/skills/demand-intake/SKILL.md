---
name: demand-intake
description: Coleta de contexto pré-minuta para notificação extrajudicial — partes, fatos, fundamento, alavancagem, BATNA, filtros de sigilo profissional — gravada em intake.md estruturado que o skill demand-draft lê. Use quando o(a) usuário(a) quiser preparar notificação extrajudicial, rodar intake antes de redigir, ou capturar contexto para notificação de cobrança, interpelação para cura, cessar-e-desistir, distrato/rescisão trabalhista ou notificação de preservação documental.
argument-hint: "[título] [--full]"
---

# /demand-intake

> **Adaptação BR não oficial.** Tipos típicos de notificação no BR: cobrança (CC 397, 200 ss.); interpelação para cura (CC 474; cláusula resolutiva expressa); cessar-e-desistir (LPI 209; Lei 9.610/1998; CDC); distrato/rescisão trabalhista (CLT 477-484); preservação documental (boa-fé CC 422; CPC 400 — presunção pela recusa); notificação judicial (CPC 726-729). **Não há FRE 408**: propostas de acordo fora de mediação (Lei 13.140/2015) podem virar prova (CPC 389 — confissão). EOAB CED art. 44: vedação a ameaça temerária; tom proporcional. Conferir prescrição (CC 189-206; CDC 27; CLT 11) antes de notificar — notificação interrompe a prescrição (CC 202 II).

1. Load `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → demand-letter practice, landscape, risk calibration.
...
5. Write `~/.claude/plugins/config/claude-for-legal/contencioso/demand-letters/[slug]/intake.md`.
6. Confirm with user: "Intake saved. Run `/contencioso:demand-draft [slug]` when ready."
...
- Desired outcome dollar value ≥ the medium-severity band from `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` risk calibration
- Counterparty is a customer, competitor, or frequent adversary per `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` landscape
- User ran with `--full`
...
`[type]-[counterparty-short]-[yyyy-mm]`. Confirm uniqueness in `~/.claude/plugins/config/claude-for-legal/contencioso/demand-letters/`.

### `~/.claude/plugins/config/claude-for-legal/contencioso/demand-letters/[slug]/intake.md`
...
End with:
> Intake saved. When ready: `/contencioso:demand-draft [slug]`
...
- Run the conflicts check. If the counterparty is a customer or known entity, flag that this should clear conflicts (per `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`) before sending — but the check itself lives in the matter-intake workflow or outside this skill.
