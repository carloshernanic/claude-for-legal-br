...
description: Triagem de ofício ou requisição recebida pela empresa — classifica, analisa escopo/onerosidade/sigilo, cruza com a carteira, e produz framework de impugnação, plano de cumprimento e calendário de prazos. Use quando o(a) usuário(a) disser "recebemos ofício", "recebemos requisição judicial", "fomos intimados como terceiro", ou compartilhar ofício/requisição/intimação para avaliar.
argument-hint: "[caminho-para-ofício] [--slug=slug-customizado]"
---

# /subpoena-triage

> **Adaptação BR não oficial — "subpoena" não existe no BR.**
>
> No direito brasileiro **não há "subpoena" no sentido US** (writ de juízo ou de advogado(a) para terceiro produzir documento ou comparecer). Os instrumentos funcionalmente equivalentes:
>
> | Original (US) | Equivalente funcional BR |
> |---|---|
> | Subpoena duces tecum (third-party docs) | **Ofício judicial** (CPC art. 438) — juízo requisita a terceiro; **exibição** (CPC 396-404) entre partes |
> | Subpoena ad testificandum (third-party depo) | **Intimação para testemunho** (CPC 455) — testemunha intimada para audiência |
> | Civil Investigative Demand (CID) — DOJ | **Ofício de regulador / autoridade administrativa** — CADE (Lei 12.529/2011 arts. 13 ss.), CVM (Lei 6.385/1976; Resolução CVM 45 — termo de compromisso), BCB (Lei 4.595/1964), ANPD (LGPD art. 55-J), MP (Lei 8.625/1993 art. 26 — requisição), TCU |
> | Grand jury subpoena | **Inquérito policial / civil (MP)** — CPP arts. 5 ss.; **requisição do MP** (LC 75/1993 art. 8º) — em investigação criminal, escalonar imediatamente para criminalista; em investigação civil pública, MP pode requisitar (Lei 7.347/1985 art. 8º §1º; Lei 8.625/1993 art. 26) |
> | Quash subpoena (FRCP 45) | **Mandado de segurança** contra ato coator (CF 5º LXIX; Lei 12.016/2009) se ilegal/abusivo; **impugnação** ao próprio juízo; **embargos de terceiro** (CPC 674) se for bens |
> | Privilege objection | Recusa fundamentada em **sigilo profissional** (EOAB 7º XIX; CPC 388 II; CPP 207) ou **segredo de justiça** / **sigilo bancário-fiscal** (LC 105/2001; CTN 198) — invocação caso-a-caso |
> | Costs of compliance | **Ressarcimento de despesas a terceiro** (CPC 95); custas como sucumbência (CPC 82-91) |
> | Document preservation triggered by subpoena | **Notificação interna de preservação** — boa-fé (CC 422); recusa = presunção (CPC 400) |
>
> **Tipos típicos de ofício recebido no BR:**
> 1. **Ofício judicial cível** (CPC 438) — juízo requisita documentos a terceiro.
> 2. **Intimação para testemunho** (CPC 455) — pessoa física como testemunha.
> 3. **Ofício de regulador** (CADE, CVM, BCB, ANPD, ANVISA, ANATEL, ANEEL, MPT) — prazo legal próprio, sanção administrativa por descumprimento.
> 4. **Requisição do MP** (Lei 8.625/1993 art. 26) — civil pública (Lei 7.347/1985 art. 8º §1º) ou criminal (LC 75/1993 art. 8º).
> 5. **Ofício da Receita Federal** (CTN 197; LC 105/2001) — sigilo fiscal tem regime próprio.
> 6. **Inquérito policial** (CPP 5 ss.) — escalar a criminalista; sigilo profissional é gatilho.
> 7. **CPI** (Lei 1.579/1952; CF art. 58 §3º) — poderes de autoridade judicial; quebra de sigilo exige decisão fundamentada.
>
> **Pontos críticos BR:**
> - **Prazo:** judicial cível geralmente 5 dias úteis (CPC 218 + 219); reguladores têm prazo próprio (CADE 5-30 dias úteis dependendo do tipo); MP prazo "razoável" (Lei 8.625 art. 26 §3º — 10 dias úteis salvo justificativa).
> - **Sigilo:** invocação caso-a-caso. **Não há privilege log formal** — listar internamente para fundamentar a invocação.
> - **Recusa:** pode caracterizar crime de desobediência (CP 330) ou ato atentatório à dignidade da justiça (CPC 77 IV; CPC 380 parágrafo único — terceiro descumpre = multa).
> - **LGPD:** se a requisição envolve dados pessoais, comunicar titulares **só** quando legal (LGPD art. 9º §2º; art. 18); ANPD pode ser consultada.
> - **Sigilo bancário/fiscal:** quebra exige decisão judicial fundamentada (LC 105 art. 1º §4º; STF RE 601.314).
> - **Aviso ao seguro:** se a requisição sinaliza disputa material, conferir cláusula da apólice (CC 771).

1. Read the subpoena from provided path.
2. Classify (third-party-docs / third-party-depo / party / CID / grand-jury).
3. If grand jury → stop, escalate per `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`. Otherwise continue.
4. Load `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml` for cross-check. Load `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → landscape, privilege conventions, escalation norms.
...
7. Write `~/.claude/plugins/config/claude-for-legal/contencioso/inbound/[slug]/triage.md`. Copy or link subpoena to `~/.claude/plugins/config/claude-for-legal/contencioso/inbound/[slug]/incoming.[ext]`.
...
- `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml` — for related matter lookup and legal hold status
- `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → landscape (regulators we deal with), house privilege conventions, escalation norms
...
- Privilege log will be required — flag the format per `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`
...
Output: `~/.claude/plugins/config/claude-for-legal/contencioso/inbound/[slug]/triage.md`.
...
**Privilege log format required:** [per `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`]
...
**Before responding to the subpoena (serving objections, producing documents, appearing for deposition, or filing a motion to quash — any substantive response to the issuing party or court):** Read `## Who's using this` in `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`. If the Role is Non-lawyer:
...
- If classified as **grand jury subpoena** → stop, flag for escalation per `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`, do not proceed with standard triage.
