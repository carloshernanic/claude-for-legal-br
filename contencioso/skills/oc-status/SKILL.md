---
name: oc-status
description: Gera minutas de e-mail semanal de status ao escritório externo em toda a carteira ativa — markdown por matéria, mais minutas no Gmail quando o MCP estiver disponível. Use quando o(a) usuário(a) pedir solicitações de status, check-ins semanais com a banca externa, ou quiser e-mails de status por matéria redigidos a partir do log da carteira.
argument-hint: "[--all | --slug=foo | --no-gmail]"
---

# /oc-status

> **Adaptação BR não oficial.** "Outside counsel" = **escritório externo / banca contratada**. Convenção contratual no BR: contrato de prestação de serviços advocatícios (CC 593-609; EOAB) com cláusula de êxito, hora, fixa ou misto; provimento CFOAB sobre publicidade. Reporting esperado da banca BR inclui: andamento processual via PJe/e-SAJ/e-Proc; cumprimento de prazos (CPC 218-235); custas e despesas (CPC 82-91); provisão sugerida (CPC 25 — companhias abertas); estratégia recursal (apelação CPC 1009; agravo de instrumento CPC 1015; embargos CPC 1022; especiais CPC 1029; extraordinários CPC 1029); honorários por sucumbência (CPC 85). Reporting deve respeitar **sigilo profissional** (EOAB 7º XIX) — material recebido da banca é privilegiado. Aviso a seguradora segue CC 771. Diligência LGPD para guarda de dados pessoais do cliente pela banca (LGPD art. 7º IX — exercício regular de direitos em processo).

To run weekly, set a recurring reminder to invoke `/contencioso:oc-status`. Automated scheduling requires a scheduled-tasks integration, which is not bundled.

1. Load `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml`, filter per default rules (or per flags).
2. Load `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → outside counsel directive style, signer defaults, budget posture.
3. Follow the workflow and reference below.
4. For each matter in scope: read `matter.md` + `history.md`, draft per-matter email.
5. Write markdown to `~/.claude/plugins/config/claude-for-legal/contencioso/oc-status/[YYYY-MM-DD]/[slug].md`.
6. If Gmail MCP authenticated: create Gmail drafts. Else: markdown-only, note in summary.
7. Write `~/.claude/plugins/config/claude-for-legal/contencioso/oc-status/[YYYY-MM-DD]/_summary.md` — what ran, what was skipped and why.
...
- `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml` — the filtering and field source
- `~/.claude/plugins/config/claude-for-legal/contencioso/matters/[slug]/matter.md` — matter context (current posture, open questions)
- `~/.claude/plugins/config/claude-for-legal/contencioso/matters/[slug]/history.md` — recent events to inform what to ask about
- `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → outside counsel directive style, signer name/email, budget posture
...
**Subject:** per house convention (from `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` outside counsel directive style; fallback: `[Matter: [matter name]] — Weekly status update`)
...
4. **Budget** — [monthly / quarterly / on-request per `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` budget posture]. Where are we against [budget authorization from matter.md]? Any variance to flag?
...
[Signoff — name, role, contact. From `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` signer default for OC directives.]
```

Adapt tone per `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` outside counsel directive style — some shops are "dear counsel" formal; others are first-name-and-bullets. Match.

## Output

### Markdown drafts

Write to: `~/.claude/plugins/config/claude-for-legal/contencioso/oc-status/[YYYY-MM-DD]/[slug].md`
...
**From:** [signer name / email from `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`]
**Subject:** [subject line]

> The work-product header above applies to this internal record. The outgoing email body below goes to outside counsel on a retained matter, which is itself a privileged communication — apply the house privilege marking (`~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` privilege conventions) at the top of the email sent, typically `Privileged & Confidential — Attorney-Client Communication / Attorney Work Product`, not this internal work-product header.
...
After processing all matters, write `~/.claude/plugins/config/claude-for-legal/contencioso/oc-status/[YYYY-MM-DD]/_summary.md`:
...
This skill is designed to run weekly. Automated scheduling requires a scheduled-tasks integration that is not bundled with the plugin. To run weekly, set a recurring reminder to invoke `/contencioso:oc-status` — e.g., Monday morning on your calendar.
...
- **Enforce a minimum template.** If the house tone is "one line, first name, done," the draft honors that and skips the bulleted structure. Match `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`.
