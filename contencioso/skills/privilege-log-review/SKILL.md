...
description: Revisão de primeira passada de classificação de sigilo profissional — fazer as chamadas óbvias e flagar as duvidosas para revisão do(a) advogado(a) sem decidir as limítrofes. Use quando o(a) usuário(a) disser "revisar a relação de documentos sigilosos", "classificar sigilo", "conferir sigilo destes documentos", ou tiver lista a triar antes de exibição ou juntada.
argument-hint: "[arquivo de relação, ou conjunto de documentos]"
---

# /privilege-log-review

> **Adaptação BR não oficial — ATENÇÃO: privilege log NÃO existe no Brasil.**
>
> No direito brasileiro **não há instituto equivalente ao "privilege log" americano** (FRCP 26(b)(5)(A) — a parte que retém produção privilegiada deve listar e descrever os documentos). O que existe:
>
> | Original (US) | Equivalente funcional BR |
> |---|---|
> | Attorney-client privilege | **Sigilo profissional** (EOAB Lei 8.906/1994 art. 7º XIX, II; CPC art. 388 II; CPP art. 207) — proteção da inviolabilidade do(a) advogado(a) e da comunicação cliente-advogado(a) |
> | Work product (FRCP 26(b)(3)) | **Não há doutrina codificada** — protege-se via sigilo profissional (EOAB 7º XIX), inviolabilidade do escritório, arquivos e correspondência |
> | Common interest / joint defense | **Não há doutrina codificada** — protege-se contratualmente (NDA, acordo de cooperação) |
> | Privilege log (FRCP 26(b)(5)(A)) | **Não há equivalente** — proteção é por **invocação caso-a-caso**, em sede de exibição (CPC 396-404) ou requisição (CPC 438) |
> | "Crime-fraud exception" | EOAB 7º §6º: sigilo cede em busca e apreensão com fundado indício de prática de crime pelo(a) próprio(a) advogado(a); doutrina restritiva |
> | Waiver of privilege | Renúncia tácita: divulgação ampla pode caracterizar quebra; jurisprudência STJ é restritiva |
>
> **Por que este skill ainda é útil no BR:**
>
> 1. **Triagem interna pré-decisão.** Antes de o(a) advogado(a) decidir invocar sigilo numa exibição (CPC 397) ou em ofício (CPC 438), classificar internamente o conjunto documental ajuda a fundamentar a invocação.
> 2. **Demanda transfronteiriça.** Quando a empresa enfrenta discovery americano (28 USC 1782; Hague Evidence Convention; FRCP) ou disclosure inglês (CPR 31), a triagem interna brasileira alimenta o privilege log estrangeiro. Sigilo profissional BR pode não ser reconhecido em jurisdição estrangeira; **confirmar com counsel local**.
> 3. **Compliance interno.** Empresa precisa saber onde mora material sob sigilo profissional para fins de retenção, acesso, e resposta a autoridade.
>
> **Categorias BR para classificação:**
>
> - **Sigilo profissional (EOAB 7º XIX) — alta proteção.** Comunicação com advogado(a) inscrito(a) na OAB sobre matéria jurídica; minutas elaboradas por/para advogado(a); memos jurídicos; pareceres.
> - **Sigilo profissional em contemplação de litígio — proteção robusta.** Material elaborado em previsão de demanda — equivalente funcional aproximado ao work product, mas sem doutrina codificada.
> - **Segredo de justiça (CPC 189) — proteção por decisão judicial.** Documentos em autos sob segredo.
> - **Sigilo bancário / fiscal (LC 105/2001; CTN 198) — proteção legal própria.** Dados financeiros, declarações fiscais.
> - **Dados pessoais sensíveis (LGPD art. 11).** Origem racial, saúde, biometria — base legal LGPD para tratamento.
> - **Segredo de indústria/comércio (LPI art. 195).** Informações técnicas e comerciais.
> - **Fora de sigilo.** Comunicação com contraparte sem advogado(a); peças protocoladas; correspondência regulatória pública; documentos publicados.
> - **Limítrofe.** Conteúdo misto jurídico/negocial; comunicação com não-advogado(a) interno(a); contemplação de litígio incerta. **Flagar para revisão do(a) advogado(a).**
>
> **Risco crítico no BR:** sub-classificar (falhar em proteger material sob sigilo) é **porta de uma via** — material exposto não se desexpõe. Super-classificar é **porta de duas vias** — advogado(a) corrige na revisão. Default: super-classificar.

1. Load `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → review protocol, priv log format.
...
**Matter context.** Check `## Matter workspaces` in the practice-level CLAUDE.md. For contencioso the default is `Enabled: ✓` — every case gets its own matter workspace. If `Enabled` is `✗` (you turned it off because you work one case at a time), skip the rest of this paragraph and use practice-level context. If enabled and there is no active matter, ask: "Which matter is this for? Run `/contencioso:matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.claude/plugins/config/claude-for-legal/contencioso/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.
...
`~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → privilege log format, review protocol.

**Conflicts gate — unbypassable.** Before reviewing a privilege log, check `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:

> "I don't see [matter slug] in the matter log. Run `/contencioso:matter-intake` first so the conflicts check runs and the matter workspace is set up. I won't review a privilege log on a matter that hasn't been intaken — the conflicts check is the gate, and a privilege log review is work product that needs to live in the matter file."
