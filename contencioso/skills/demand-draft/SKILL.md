---
name: demand-draft
description: Redige notificação extrajudicial a partir de intake completo, com gate de sigilo profissional / admissão / renúncia / risco de prova, saída .docx, checklist pós-envio e oferta de criar matéria. Use quando o(a) usuário(a) disser "redija a notificação", "escreva a carta de [tipo]", ou tiver intake de notificação extrajudicial pronto para virar minuta enviável.
argument-hint: "[slug] [--skip-gate] [--version=N]"
---

# /demand-draft

> **Adaptação BR não oficial — alerta crítico sobre sigilo de tratativas.**
>
> No direito brasileiro **não existe equivalente direto ao FRE 408 (EUA)** que dê sigilo amplo a propostas de acordo em correspondência ordinária. O sigilo formal de tratativas existe APENAS em:
> - **Mediação e conciliação** (CPC art. 166 §3º; Lei 13.140/2015 art. 30)
> - **Arbitragem** (Lei 9.307/1996 art. 13 §6º; Resolução CAM-CCBC e regulamentos das câmaras)
>
> Fora desses canais, **propostas de acordo em notificação extrajudicial podem ser juntadas como prova** — para demonstrar ciência da contraparte, constituir em mora, confissão parcial (CPC 389), ou caracterizar reconhecimento de dívida (CC 202 VI — interrompe prescrição). A skill DEVE alertar o(a) advogado(a) antes de incluir valores de oferta numa notificação fora de mediação/conciliação formal. Marcações como "sem prejuízo" / "without prejudice" têm valor moral / persuasivo, mas **não criam sigilo automático** no direito brasileiro.
>
> | Original (US) | Equivalente funcional BR |
> |---|---|
> | Demand letter | Notificação extrajudicial (CC 397; CPC 726-729 — notificação judicial); interpelação |
> | Cease-and-desist | Cessar e desistir (LPI 209; Lei 9.610/1998 art. 102; CDC art. 4º) |
> | FRE 408 settlement privilege | NÃO existe broad equivalente; só CPC 166 §3º (mediação/conciliação) e Lei 13.140/2015 art. 30 |
> | Attorney work product / FRCP 26(b)(3) | Sigilo profissional (EOAB art. 7º XIX); CPC 388 II; CPP 207 — invocação caso-a-caso, sem privilege log |
> | Tolling agreement | Acordo de suspensão da prescrição (CC 202 — atos interruptivos taxativos; CC 199 — não corre prescrição) |
> | Statute of limitations / SOL | Prescrição (CC 189-206; CDC 27 — 5 anos; CLT 11 — 5 anos / 2 do término); decadência |
> | Pre-suit demand requirement | Pré-requisitos: notificação para constituir em mora (CC 397); interpelação judicial (CPC 726); notificação prévia em alguns contratos (CC 474, cláusula resolutiva) |
> | Litigation hold | Ordem de preservação documental — boa-fé (CC 422); CPC 396-404 (exibição); LGPD (guarda lícita); presunção de veracidade pela recusa/destruição (CPC 400) |
> | Rule 11 | Litigância de má-fé (CPC 80) — multa de 1-10% do valor da causa (CPC 81) |
> | Disclosure (insurance / financial) | Aviso ao seguro (apólice + CC 771); divulgação CVM (Resolução CVM 80 — fato relevante) |
> | Cost shifting / fee shifting | Sucumbência (CPC 85 — honorários 10-20%) |
>
> **Gate revisto BR:** antes de redigir, conferir (a) status de sigilo profissional EOAB 7º XIX, (b) risco de admissão / confissão (CPC 389 — confissão extrajudicial é prova; CC 212 IV), (c) renúncia tácita de direitos (CC 110, 191), (d) constituição em mora (CC 397), (e) tom (proporcionalidade, EOAB CED art. 44 — vedação de captação ou ameaça temerária), (f) acuidade factual (litigância de má-fé CPC 80).

1. Load `~/.claude/plugins/config/claude-for-legal/contencioso/demand-letters/[slug]/intake.md`. Refuse if missing or strategic block empty (for material demands).
2. Load `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → demand-letter practice, house style, seed-doc table.
...
5. Template select: seed doc if provided in `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`; else soft template for the demand type.
6. Draft in-chat for review. Iterate until user approves.
7. Write `~/.claude/plugins/config/claude-for-legal/contencioso/demand-letters/[slug]/draft-v[N].docx` using the docx skill.
8. Write `~/.claude/plugins/config/claude-for-legal/contencioso/demand-letters/[slug]/checklist.md` (post-send checklist).
...
- `~/.claude/plugins/config/claude-for-legal/contencioso/demand-letters/[slug]/intake.md` — required; refuse to proceed if missing
- `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → Demand-letter practice (seed-doc paths, insurance-tender timing, materiality threshold for matter creation), house style (privilege markings, outside counsel directive format for tone reference). **Tone, compliance period, marking, and signer come from `## Posture for this matter` — they are matter-level, not practice-level.**
- `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml` — to check for existing related matters (same counterparty) and offer cross-link
...
Check `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → Demand-letter practice → seed-doc table for the intake's demand type.
...
7. **Privilege markings per house style.** Apply `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` privilege conventions exactly.

## Output

### Primary: `~/.claude/plugins/config/claude-for-legal/contencioso/demand-letters/[slug]/draft-v[N].docx`
...
### `~/.claude/plugins/config/claude-for-legal/contencioso/demand-letters/[slug]/checklist.md` — the post-send checklist
...
**Before the letter is sent (the consequential act):** Read `## Who's using this` in `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md`. If the Role is Non-lawyer:
...
If material: `/contencioso:matter-intake` with `source: demand-letter` pre-populated from this intake.
```
...
- **Default yes if ANY of:**
  ...
  - Desired outcome $$ ≥ `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` medium-severity band
...
> Create a tracked matter in `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml`? (default: [yes/no])
...
If user declines: update intake `status: drafted` (later `sent` when user confirms). The record stays in `~/.claude/plugins/config/claude-for-legal/contencioso/demand-letters/` only.
