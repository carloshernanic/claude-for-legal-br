---
name: claim-chart
description: Constrói ou revisa quadro de elementos — quadro de reivindicações de patente (infração de patente conforme LPI Lei 9.279/1996 art. 42, nulidade conforme LPI art. 56, ou revisão) ou quadro civil de elementos para qualquer causa de pedir ou matéria de defesa — com cada célula vinculada à fonte e detecção de lacunas como saída prioritária. Use quando o(a) usuário(a) pedir quadro de reivindicações, quadro de elementos, quadro de prova, mapeamento elemento-por-elemento, ou perguntar "o que falta para provar [a causa de pedir]".
argument-hint: '[--patent | --civil] [--infringement | --invalidity | --review] [--claim <n>] [--count <nome>] [--target <slug>]'
---

# /claim-chart

> **Adaptação BR não oficial.** O original cobre patente (US 35 USC §§ 101/102/103/112/271 + Markman) e civil US (CACI/NYPJI; Twombly/Iqbal; FRCP 56). Mapa de equivalência:
>
> | Original (US) | Equivalente funcional BR |
> |---|---|
> | 35 USC §271 (infringement) | LPI (Lei 9.279/1996) arts. 42, 183-186; ação de abstenção/indenização (CPC + LPI 207-210) |
> | 35 USC §§ 102/103 (anticipation/obviousness) | Nulidade administrativa de patente (LPI arts. 50-56); ação de nulidade (LPI 56, foro Federal SJ-RJ contra o INPI) |
> | 35 USC §112 (definiteness, written description) | LPI art. 24 (descrição), art. 25 (reivindicações) |
> | Markman hearing | Sem instituto correspondente — interpretação de reivindicações pelo juízo da SJ-RJ (vara especializada PI) ou TRF |
> | Doctrine of Equivalents (DOE) | LPI art. 186 — infração por equivalentes / "produto/processo equivalente" |
> | Inter Partes Review / IPR (PTAB) | Processo administrativo de nulidade no INPI (LPI 50-55) |
> | FRCP 8/12(b)(6) / Iqbal-Twombly plausibility | CPC arts. 319 (requisitos da inicial), 330 (indeferimento), 332 (improcedência liminar); inépcia (CPC 330 §1º) |
> | FRCP 56 / MSJ | Julgamento antecipado do mérito (CPC 355) ou julgamento antecipado parcial (CPC 356) |
> | Pattern jury instructions (CACI/NYPJI) | Não há júri cível no Brasil. Os "elementos" derivam diretamente das normas: CC arts. 186, 187, 927 (responsabilidade civil); CC art. 389 (inadimplemento); CC arts. 138-155 (vícios do consentimento — fraude/erro/dolo); CDC art. 14 (defeito do serviço); CLT arts. 482-483 (justa causa); Lei 7.347/1985 (ACP) |
> | Affirmative defenses | Preliminares (CPC 337 — coisa julgada, ilegitimidade, conexão, litispendência, prescrição/decadência) + matérias de mérito (CPC 350) |
> | Restatement (Torts/Contracts) | CC (Lei 10.406/2002); CDC; CLT |
> | Rule 11 / patent local rules | Litigância de má-fé (CPC 80); ônus da prova (CPC 373); honorários por sucumbência (CPC 85) |
> | Statute of limitations | Prescrição (CC 189-206; CDC 27 — 5 anos; CLT 11 — 5 anos / 2 do término); decadência |
> | Pin cite (column:line, page, Bates) | Referência a fls., ID PJe, evento processual, art./§ da norma; jurisprudência por nº STJ/STF/TST |
>
> No modo `--patent`, ações de nulidade contra patente concedida pelo INPI vão à **Justiça Federal SJ-RJ** (LPI 57); ações de infração entre particulares vão à **Justiça Estadual** (varas empresariais) salvo conexão com nulidade.
>
> No modo `--civil`, "elementos" são os requisitos legais da causa de pedir conforme **norma** (não jury instruction). Carregar do CC, CDC, CLT, ou lei especial; não inventar elementos a partir de pattern instructions americanos.

1. Load `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → role, work-product header, decision posture, document storage.
...
Check `## Matter workspaces` in the practice-level CLAUDE.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `/contencioso:matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` — especially the case theory, the pleading / complaint (for the elements actually alleged), the jurisdiction, any Markman order or stipulated constructions (patent mode), and the phase of the case. Write outputs to the matter folder at `~/.claude/plugins/config/claude-for-legal/contencioso/matters/<matter-slug>/claim-charts/`. Never read another matter's files unless `Cross-matter context` is `on`.

---

## Load context

- `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` → role, work-product header, decision posture, document storage, case-theory scaffolding
...
>
> **Two choices:**
> - Run `/contencioso:cold-start-interview` (2 minutes) to configure your profile, then I'll run this tailored to YOUR practice.
...
> "That was a generic run against default assumptions. Run `/contencioso:cold-start-interview` to get output calibrated to YOUR practice — your risk calibration, your landscape, your house style. 2 minutes."

**Conflicts gate — unbypassable.** Before building a claim chart, check `~/.claude/plugins/config/claude-for-legal/contencioso/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:

> "I don't see [matter slug] in the matter log. Run `/contencioso:matter-intake` first so the conflicts check runs and the matter workspace is set up. I won't build a claim chart on a matter that hasn't been intaken — the conflicts check is the gate."
...
Prepend the work-product header from `~/.claude/plugins/config/claude-for-legal/contencioso/CLAUDE.md` `## Outputs`.
...
Ask which the team works in. Use the pattern from `societario`'s `tabular-review` skill — same cell-level citation model, same state-based color coding, same `Verified` column, same schema sheet:
...
If matter workspaces enabled and a matter is active: `~/.claude/plugins/config/claude-for-legal/contencioso/matters/<matter-slug>/claim-charts/`. Otherwise: `~/.claude/plugins/config/claude-for-legal/contencioso/claim-charts/`. Surface the path. Append a one-line entry to the matter's `history.md`.
...
- `propriedade-intelectual:infringement-triage` (patent mode) — the first-pass flag list. This skill is the full chart that comes next.
- `propriedade-intelectual:fto-triage` — FTO uses the same mechanics from the potentially-accused posture. If evaluating own product vs. a third-party patent, route to FTO and use this skill's format.
- `societario:tabular-review` — the underlying cell-level citation and verification-state pattern. A claim / element chart is a specialized tabular review.
- `contencioso:chronology` — the chronology is the timeline; the element chart is the proof matrix. A chronology entry often becomes a cell's evidence cite.
- `contencioso:deposition-prep` — a `needs-discovery` cell often becomes a depo topic. After a depo, new testimony fills cells.
- `contencioso:brief-section-drafter` — an MSJ brief's fact section is often built directly off the supported rows of an element chart.
