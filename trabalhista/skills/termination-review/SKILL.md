...
description: >
  Revisão de rescisão — detecção de bandeiras de alto risco (estabilidades:
  gestante, CIPA, acidentário, dirigente sindical, pré-aposentadoria se
  CCT/ACT), verbas rescisórias e prazo de pagamento por categoria/CCT.
  Regras específicas (Lei 12.506/2011 aviso prévio proporcional; CLT art. 477;
  homologação dispensada para vínculos com mais de 1 ano pós-Reforma 13.467;
  acordo extrajudicial CLT 855-B; multa FGTS 40%) são pesquisadas por
  revisão, não armazenadas. Use quando o(a) usuário(a) disser "revisar uma
  rescisão", "podemos dispensar essa pessoa", "term review", ou descrever
  cenário de rescisão.
...
> - **At-will / wrongful termination** → **NÃO EXISTE AT-WILL NO BRASIL.** Toda rescisão sem justa causa de contrato por prazo indeterminado é direito potestativo do empregador (CLT art. 477), mas dispara verbas rescisórias completas. Modalidades: sem justa causa (verbas integrais), com justa causa (CLT art. 482 — verbas reduzidas), pedido de demissão (CLT art. 487 — empregado deve aviso prévio), acordo (CLT art. 484-A — verbas pela metade pós-Reforma), culpa recíproca (CLT art. 484), rescisão indireta (CLT art. 483 — falta grave do empregador).
> - **Final-pay timing (state-by-state)** → CLT art. 477 §6º: **10 dias corridos** a partir do término do contrato (aviso prévio cumprido ou indenizado). Atraso → multa do art. 477 §8º (um salário do(a) empregado(a)).
> - **Severance (formula/discretionary)** → verbas obrigatórias sem justa causa: **aviso prévio** (CLT 487; proporcional Lei 12.506/2011 — 3 dias por ano de serviço, mín. 30, máx. 90), **13º proporcional** (Lei 4.090/62), **férias proporcionais + 1/3** (CF 7º XVII), **férias vencidas + 1/3** se houver, **FGTS depositado + multa 40%** (Lei 8.036/90 art. 18 §1º; LC 110/2001), saque do FGTS + seguro-desemprego (Comunicação de Dispensa). Severance "discretionary" adicional vira **plus indenizatório** atípico; cuidado com habitualidade (Súmula 51 TST).
> - **Release / OWBPA consideration periods (40+)** → não há equivalente. **Quitação tem eficácia liberatória limitada** (OJ 270 SDI-I TST — só o expressamente quitado). Veículo mais robusto: **acordo extrajudicial homologado pela Justiça do Trabalho** (CLT art. 855-B, advogados distintos para cada parte). Plano de demissão voluntária com adesão em condições objetivas pode quitar de forma ampla (Tema 152 STF — válida quitação ampla em PDV negociado com sindicato).
> - **Mass-layoff / WARN Act / mini-WARN** → **dispensa coletiva**: STF Tema 638 e CLT art. 477-A (pós-Reforma) — negociação coletiva prévia é exigida (entendimento STF/TST). Critério quantitativo varia conforme CCT/ACT.
> - **NDA / non-disparagement post-termination (NLRA / state)** → cláusulas de confidencialidade pós-contratuais são válidas; non-disparagement deve ser equilibrada (não pode impedir denúncia ao MPT — CF 5º XXXIII, XXXIV, XXXV).
> - **FLSA back-pay (regular rate, 0.5x premium, liquidated damages)** → horas extras CLT arts. 58–75; adicional mínimo 50% (CF 7º XVI) — pode ser maior por CCT; reflexos em DSR (Lei 605/49), 13º, férias, FGTS, aviso prévio; prescrição bienal a contar da rescisão + quinquenal das parcelas (CF 7º XXIX).
> - **State / state-by-state** → UF + **categoria sindical + CCT/ACT vigente**.
> - **Protected leave (FMLA/CFRA/PFL etc.)** → estabilidades: gestante (ADCT 10 II b; Súmula 244 TST — confirmação da gravidez até 5 meses pós-parto), CIPA titular/suplente (CLT 165; ADCT 10 II a), acidentário (Lei 8.213/91 art. 118 — 12 meses pós-cessação do auxílio-doença acidentário), dirigente sindical (CF 8º VIII; CLT 543 §3º), CCP (CLT 625-B §1º), reabilitado/PCD (Lei 8.213 art. 93 §1º), pré-aposentadoria (conforme CCT/ACT).
> - **EEOC / discrimination** → Lei 9.029/1995 (proíbe dispensa discriminatória — gênero, raça, estado civil, gravidez, idade); reintegração com dobra (art. 4º). Lei 14.611/2023 (igualdade salarial).
> - **Whistleblower** → CF art. 5º XXXIV; Lei 13.608/2018 (canal de denúncia / ouvidoria); proteção contra dispensa retaliatória após denúncia ao MPT.
> - **Exempt misclassification flag** → pejotização (CLT art. 9º — fraude); enquadramento indevido em CLT art. 62 (cargo de confiança ou teletrabalho por produção) sem suporte fático → reconhecimento da relação de emprego + horas extras + reflexos.

# /termination-review

1. Load `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → termination review triggers, high-risk flags, severance practice, jurisdiction rules.
...
`~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → termination review triggers, high-risk flags, standard severance,
jurisdiction table.

## Output header

Prepend the work-product header from `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → `## Outputs` (it differs by user role — see `## Who's using this`). Match the memo format from seed term memos referenced in that config where one exists. The work-product header is always first.
...
This is the most important step. Check every flag from `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md`. Default
set:
...
> `/trabalhista:wage-hour-qa` for a classification check — a misclassified
...
`/trabalhista:wage-hour-qa` for the actual duties-and-salary test.
...
**Any flag fires → escalate per `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` before the term proceeds.** Not
after. Before.
...
Per `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → standard severance:
...
> **Jurisdiction assumption.** This review assumes the employee's jurisdiction as stated in Step 1 and any defaults from `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → Jurisdictional footprint. Employment rules, final-pay timing, release requirements, and notice obligations vary materially by jurisdiction. If the employee works in a different state or country, or if choice-of-law is contested, this analysis may not apply as written.

Match the memo format from seed term memos referenced in `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md`. If none:
...
**Before producing a "Go" recommendation or a term-day checklist marked ready:** Read `## Who's using this` in `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md`. If the Role is **Non-lawyer**:
