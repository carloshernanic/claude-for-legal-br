...
description: >
  Classifica um engajamento de trabalhador(a) proposto — CLT, PJ/autônomo,
  terceirizado, estagiário, jovem aprendiz, temporário (Lei 6.019/74),
  intermitente (CLT 452-A) — rodando os testes aplicáveis (CLT art. 3º:
  pessoalidade, não-eventualidade, onerosidade, subordinação; Súmula 331 TST
  para terceirização; CLT art. 442-B para autônomo exclusivo; Lei 13.429/2017
  para terceirização irrestrita) e sinalizando lacunas entre o arranjo
  pretendido e o que os fatos efetivamente sustentam. Uso prospectivo
  apenas. Use quando alguém disser "queremos trazer um(a) PJ", "isto é
  terceirizado ou temporário", "como devemos classificar essa pessoa", ou
  descrever um arranjo proposto.
argument-hint: "[descreva o arranjo proposto, ou comece e farei perguntas]"
---

> **Adaptação BR:** Skill adaptada do regime US para o regime brasileiro. Mapeamentos-chave:
> - **W-2 employee (federal payroll tax/FLSA)** → **vínculo de emprego CLT** (CLT art. 3º — requisitos: pessoalidade, não-eventualidade, onerosidade, **subordinação jurídica**). Caracterizado o vínculo, descumprimento → CLT art. 9º (nulidade dos atos para desvirtuar — fraude).
> - **1099 / Independent Contractor (federal/state tests — ABC, economic realities, common-law)** → **autônomo** (CC art. 593; CLT art. 442-B — autônomo exclusivo admitido pós-Reforma 13.467/2017 desde que sem subordinação). **PJ** (pessoa jurídica prestadora) — análise dos requisitos reais (Súmula 331 TST; CLT art. 9º). "Pejotização" é o termo para mascarar vínculo via PJ.
> - **Temp via staffing agency / co-employment** → **trabalho temporário** Lei 6.019/74 (até 180 dias prorrogáveis por 90; vagas para acréscimo de demanda ou substituição de pessoal). Empresa tomadora responde subsidiariamente. **Terceirização** (Lei 13.429/2017 + Lei 13.467/2017 — irrestrita, inclusive atividade-fim): tomadora responde subsidiariamente (Súmula 331 IV TST) e em alguns casos solidariamente (art. 5º-A §5º da Lei 6.019/74). Cuidado: terceirizar mantendo subordinação direta = vínculo direto.
> - **Vendor / SOW (B2B)** → contrato de prestação de serviços empresa-a-empresa (CC arts. 593 ss.). Ausência de pessoalidade, não-eventualidade e subordinação é o que blinda.
> - **Strict ABC test states (CA AB5)** → não há equivalente único; o teste é os **4 requisitos do art. 3º CLT** + análise do conjunto fático ("primazia da realidade").
> - **§ 530 relief (IRS safe harbor)** → não há equivalente.
> - **Federal payroll tax / state UI / WC** → INSS (cota patronal sobre folha — Lei 8.212/91), FGTS (8% sobre remuneração — Lei 8.036/90), SAT/RAT (1-3% — risco), salário-educação, sistema S (SESI/SENAI/SESC/SENAC). Para PJ não há essas obrigações — daí o incentivo à pejotização (e o risco fiscal e trabalhista se desconstituída).
> - **State** → UF + **categoria sindical + CCT/ACT**. Algumas CCTs proíbem terceirização em determinadas atividades.
> - **Joint-employer / co-employment** → responsabilidade subsidiária (Súmula 331 IV TST) ou solidária conforme caso; grupo econômico CLT art. 2º §2º (Reforma — exige relação de coordenação ou hierarquia, integração + comunhão de interesses + atuação conjunta).
> - **Right-to-control test** → "subordinação jurídica" no BR. Critérios: ordens diretas, horário, métodos, exclusividade (fator, não requisito), inserção na atividade-fim, controle do resultado.
> - **Existing-arrangement gate** → mantém-se. **Reclassificação retroativa de PJ → CLT** expõe a: passivo trabalhista (salários + verbas + reflexos por 5 anos pré + 2 anos pós), INSS, FGTS, SAT, multas administrativas (Auditoria-Fiscal), MPT, ação civil pública. Análise privilegiada com advogado(a) externo(a).
> - **Categorias especiais a considerar como alternativas**: **estagiário** (Lei 11.788/2008 — sem vínculo se cumprida a lei: matrícula, plano de atividades, convênio); **jovem aprendiz** (CLT 428–433 — cota Lei 10.097/2000); **autônomo** (CC 593); **MEI** (LC 123/2006 art. 18-A — cuidado: prestar serviço exclusivo com pessoalidade e subordinação a uma única tomadora pode caracterizar vínculo a despeito do MEI); **representante comercial** (Lei 4.886/65); **cooperativa** (Lei 5.764/71 art. 90 — afasta vínculo se autêntica).
...
1. Load `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → jurisdictional footprint, escalation table.
...
```
/trabalhista:worker-classification
...
Read `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → jurisdictional footprint, any classification history or
prior settlements noted, escalation table, and any house classification
policy the team has recorded.

## Output header

Prepend the work-product header from `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → `## Outputs` (it differs by user role — see `## Who's using this`).
...
If `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` records the company's house classification policy, apply it
...
Escalate per `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` if any of the following, or any team-specific
...
`/trabalhista:hiring-review` to review the offer letter, restrictive
...
**Before producing a "Proceed as IC / employee / agency / vendor" final recommendation:** Read `## Who's using this` in `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md`. If the Role is **Non-lawyer**:
