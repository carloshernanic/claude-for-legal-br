...
description: >
  Referência: framework compartilhado para gerir apurações internas (assédio
  moral/sexual — Lei 14.457/2022; ética; fraude; whistleblower — Lei
  13.608/2018) da intake ao memorando final — log da apuração com cabeçalho
  de sigilo profissional (EOAB art. 7º XIX), processamento de documentos com
  needle-finding, rastreamento de cobertura de fontes, Q&A contra o log,
  redação de memorando e resumos por audiência. Carregada por
  /investigation-open, /investigation-add, /investigation-query,
  /investigation-memo e /investigation-summary; não é invocada diretamente.
user-invocable: false
---

> **Adaptação BR:** Skill adaptada do regime US para o regime brasileiro. Mapeamentos-chave:
> - **Attorney-client privilege / work product** → **sigilo profissional do(a) advogado(a)** (Lei 8.906/1994 — EOAB art. 7º XIX; CF 5º XIV; CPC art. 388; CP art. 154). Em apurações internas no BR, a proteção depende de o trabalho ser **efetivamente conduzido por advogado(a) para finalidade jurídica** — rótulo não basta. Compartilhamento com RH/DP, gestores não-jurídicos, auditoria pode comprometer a confidencialidade.
> - **Upjohn warning** → no BR não há figura idêntica. Recomendação prática: **advertência de função** antes de entrevista interna — explicar que o(a) advogado(a) representa a empresa (não o(a) empregado(a)), que o conteúdo pode ser usado pela empresa, e que o(a) empregado(a) tem direito a não falar (CF 5º LXIII por analogia — privilégio de não-autoincriminação) e a ser acompanhado(a) por advogado(a) próprio(a) (EOAB art. 7º). Registrar por escrito.
> - **EEOC charge / state agency** → **MPT** (denúncia → procedimento preparatório → inquérito civil → TAC ou ACP); **TRT** (reclamação trabalhista individual ou ACP); **Auditoria-Fiscal** (auto de infração administrativo).
> - **Title VII / ADA / ADEA / FMLA retaliation** → Lei 9.029/1995 (proibição de práticas discriminatórias — gênero, raça, estado civil, gravidez, idade, deficiência); CF 7º XX (proteção do mercado de trabalho da mulher); Lei 14.611/2023 (igualdade salarial); LBI (Lei 13.146/2015); retaliação por denúncia: dispensa retaliatória presumida em janela próxima a denúncia — risco de reintegração e indenização.
> - **Sexual harassment (Title VII)** → assédio sexual: CP art. 216-A; assédio moral: jurisprudência consolidada + CCTs; **Lei 14.457/2022** — programa de prevenção e enfrentamento do assédio sexual e demais formas de violência (CIPA+A: canais de denúncia, treinamentos, código de conduta).
> - **Whistleblower (SOX / Dodd-Frank)** → Lei 13.608/2018 (canal de denúncia / programas de compliance); Lei 12.846/2013 (Anticorrupção); proteção contra dispensa retaliatória (jurisprudência TST).
> - **Garrity (public-sector compelled statements)** → análogo aproximado: servidor público estatutário (Lei 8.112/90 para federais; estatutos estaduais/municipais) — apuração administrativa disciplinar; CF 5º LV (contraditório e ampla defesa).
> - **Weingarten (union representation in investigatory interviews)** → CCT/ACT pode prever; CF 8º (representação sindical); dirigente sindical com estabilidade pode pedir acompanhamento de delegado sindical. Verificar a CCT da categoria.
> - **NDA / non-disparagement settlement (NLRA / state restrictions)** → acordos extrajudiciais sob CLT 855-B; cláusulas de confidencialidade pós-contratuais aceitas; non-disparagement deve respeitar direito à liberdade de expressão e direito de denúncia ao MPT.
> - **LGPD (Lei 13.709/2018)**: dados pessoais coletados em apuração interna exigem **base legal** (art. 7º — execução de contrato/legítimo interesse; art. 11 para dados sensíveis — saúde mental/orientação sexual em casos de assédio); minimização; segregação de acesso; retenção limitada.
> - **Preponderance of evidence** → no juízo trabalhista: livre convencimento motivado; ônus probatório distribuído conforme CLT 818 + Súmulas TST específicas (Súmula 338 — controle de jornada; etc.); inversão do ônus em assédio é jurisprudência consolidada (TST).
> - **State** → UF + categoria sindical + CCT/ACT.

# Skill de Apuração Interna
...
Prepend the work-product header from `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → `## Outputs` (it differs by user role — see `## Who's using this`). Every file, log, memo, and summary produced by this skill opens with that header.
...
Read `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → escalation table, any investigation protocols noted.

---

## Mode 1: Open a new matter

Triggered by `/trabalhista:investigation-open` or "open an investigation"
...
`~/.claude/plugins/config/claude-for-legal/trabalhista/investigation-[matter-slug]/log.yaml`:
...
`~/.claude/plugins/config/claude-for-legal/trabalhista/investigation-[matter-slug]/sources-checklist.yaml`:

Generated from the investigation type. See sources checklist templates below.

`~/.claude/plugins/config/claude-for-legal/trabalhista/investigation-[matter-slug]/documents-reviewed.yaml`:
...
`~/.claude/plugins/config/claude-for-legal/trabalhista/investigation-[slug]/sources-checklist.yaml`.

---

## Mode 2: Add data

Triggered by `/trabalhista:investigation-add` or "add to the [matter]
investigation" or when the attorney pastes documents or interview notes.

### Step 1 — Identify the matter

If multiple investigation folders exist in `~/.claude/plugins/config/claude-for-legal/trabalhista/`, ask which matter this
...
Triggered by `/trabalhista:investigation-query` or any question
...
Triggered by `/trabalhista:investigation-memo` or "draft the memo"
or "update the memo".
...
Write the draft to `~/.claude/plugins/config/claude-for-legal/trabalhista/investigation-[slug]/memo.md`.
...
Triggered by `/trabalhista:investigation-summary` or "draft a
