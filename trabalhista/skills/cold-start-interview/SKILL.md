...
description: >
  Setup inicial — aprende seu footprint jurisdicional (UFs + categorias
  sindicais + CCTs/ACTs vigentes) e regras de escalação a partir do seu
  regulamento interno e memorandos de rescisão. Pergunta quais UFs e
  países têm empregados, lê documentos-semente, e monta uma tabela de
  escalação ciente da jurisdição/categoria. Use em instalação nova, quando
  o CLAUDE.md ainda tiver marcadores `[PLACEHOLDER]`, ou ao rerodar com
  --redo ou --check-integrations.
argument-hint: "[--redo | --check-integrations]"
---

> **Adaptação BR:** entrevista adaptada do regime US (state-by-state, at-will, FLSA, EEOC) para o regime brasileiro. Substituições-chave durante a entrevista:
> - **States** → UFs + **categorias sindicais** (a unidade de variação no BR é tanto a UF quanto a categoria + base territorial do sindicato + CCT/ACT vigente).
> - **At-will employment** → NÃO EXISTE NO BRASIL. A regra é contrato por prazo indeterminado (CLT art. 443). Não pergunte sobre "at-will language" — pergunte sobre estabilidades aplicáveis (gestante ADCT 10 II b; CIPA CLT 165; acidentário Lei 8.213/91 art. 118; dirigente sindical CF 8º VIII).
> - **FLSA exemption (exempt/non-exempt)** → controle de jornada CLT art. 62 (I — externos sem controle; II — gerentes/diretores; III — teletrabalho por produção). Horas extras: CLT arts. 58–75; adicional mínimo 50% (CF 7º XVI).
> - **Pay transparency (state laws)** → Lei 14.611/2023 (igualdade salarial; relatórios semestrais MTE) + CLT art. 461.
> - **Non-compete (state enforceability)** → jurisprudência TST exige prazo (até 2 anos), limite geográfico e contrapartida financeira.
> - **EEOC charges** → reclamação trabalhista no TRT; denúncia ao MPT; auto de infração da Auditoria-Fiscal.
> - **WARN Act / mini-WARN** → dispensa coletiva submetida a negociação coletiva prévia (STF Tema 638; CLT art. 477-A pós-Reforma 13.467/2017).
> - **OWBPA (40+ release periods)** → não há equivalente direto; quitação tem eficácia liberatória limitada (OJ 270 SDI-I TST); acordo extrajudicial homologado pela JT (CLT art. 855-B) é o veículo mais robusto.
> - **HRIS (Workday/BambooHR/Rippling/ADP)** → TOTVS, Senior, Sankhya, ADP Brasil; e-Social (eventos S-1000, S-2200, S-2299, S-2230).
> - **Outside counsel (state bar)** → advocacia trabalhista externa inscrita na OAB; consulta sindical.
> Categorias com regimes especiais a perguntar especificamente: rurais (Lei 5.889/73), domésticos (LC 150/2015), bancários (CLT 224), aeronautas (Lei 13.475/2017), aprendizes (CLT 428–433), estagiários (Lei 11.788/2008), temporários (Lei 6.019/74), intermitentes (CLT 452-A), teletrabalho (CLT 75-A–75-E).

# /cold-start-interview

1. Check `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md`. If `--check-integrations`, skip the interview — re-run only the Part 0 `What's connected?` check and rewrite the `## Available integrations` table at that config path. When probing: only report ✓ if an MCP tool call actually succeeded. Configured-but-untested connectors should be marked ⚪ with a one-line how-to for confirming. Never report ✓ based on `.mcp.json` declarations alone — that misleads users into thinking something is wired up when it isn't.
...
5. If a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.claude/plugins/cache/claude-for-legal/trabalhista/*/CLAUDE.md` but not at the config path, copy it to the config path and tell the user what was migrated.
6. Write `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md`, creating parent directories as needed.
...
Read `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md`:
...
> **`trabalhista` is for people who handle hiring, terminations, investigations, leave, policies, worker classification, and international expansion.** Not your area? `/builder-hub:related-skills-surfacer`.
>
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/trabalhista:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run /trabalhista:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> - ✗ [Integration] — not found. [Feature] will fall back to [manual alternative]. [How to connect.] If you set this up later, re-run `/trabalhista:cold-start-interview --check-integrations`.
>
...
>
> - **Review an offer letter and restrictive covenants** — e.g., "Jurisdiction check on non-compete enforceability, pay transparency, and required notices." Try: `/trabalhista:hiring-review`
> - **Termination review with risk flags** — e.g., "Severance, release, final pay timing, and high-risk indicators flagged before the decision." Try: `/trabalhista:termination-review`
> - **Classify a worker engagement** — e.g., "Employee / IC / temp / vendor — with misclassification gap analysis." Try: `/trabalhista:worker-classification`
> - **Ask a jurisdiction-aware wage/hour question** — e.g., "Multi-state workforce question routed against the jurisdictions in your footprint." Try: `/trabalhista:wage-hour-qa`
> - **Kick off international expansion** — e.g., "New country on the roadmap — plan the employment-law workstream." Try: `/trabalhista:expansion-kickoff`
> - **Open an internal investigation** — e.g., "Create the privileged workspace, start the log, route interviews." Try: `/trabalhista:investigation-open`
>
...
- If manual leave tracking: "You don't have an HRIS leave module — I'll track leaves in a register file. Use /trabalhista:log-leave to add any leaves that are currently open."
...
> "Done. Your configuration is at `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
> - Edit the file directly for a quick change
> - Run `/trabalhista:cold-start-interview --redo` for a full re-interview
> - Run `/trabalhista:cold-start-interview --check-integrations` to re-check what's connected
>
...
> - Run `/trabalhista:cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
>
