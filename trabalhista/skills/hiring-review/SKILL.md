...
name: hiring-review
description: >
  Revisa carta-proposta / contrato de trabalho e cláusulas restritivas —
  checagem jurisdicional inclusa (UF, categoria sindical, CCT/ACT, regime
  CLT vs. especial). Regras substantivas (exigibilidade de não-concorrência
  per TST, transparência salarial Lei 14.611/2023, antecedentes Súmula 568
  TST, regime de jornada CLT 62, e-Social S-2200, ASO NR-7) são pesquisadas
  por admissão, não armazenadas. Use quando o(a) usuário(a) disser "revisa
  esta proposta", "podemos usar não-concorrência aqui", "checa esta
  carta-proposta", "contratando em [UF/categoria]", ou anexa uma proposta.
...
> - **At-will employment** → **NÃO EXISTE NO BRASIL.** NUNCA recomende cláusula "at-will" em contrato brasileiro. A regra é contrato por prazo indeterminado (CLT art. 443). Pode haver contrato a prazo determinado em hipóteses específicas (CLT art. 443 §2º: serviço transitório, atividade empresarial transitória, contrato de experiência até 90 dias). Contrato intermitente: CLT art. 452-A.
> - **FLSA exempt/non-exempt** → CLT art. 62 (I — externos sem controle de jornada; II — gerentes e diretores com gratificação de função; III — teletrabalho por produção pós-Reforma 13.467/2017). Cuidado: enquadrar empregado(a) no art. 62 quando os fatos não suportam expõe a passivo de horas extras com prescrição quinquenal das parcelas (CF 7º XXIX).
> - **Salary threshold (federal/state)** → piso da categoria via CCT/ACT; salário mínimo CF 7º IV; Lei 14.611/2023 (transparência salarial / relatórios semestrais MTE).
> - **Pay transparency (state laws)** → Lei 14.611/2023; CLT art. 461 (igualdade salarial — função idêntica, mesma localidade, mesmo empregador).
> - **Salary-history limits / ban-the-box** → LGPD (Lei 13.709/2018) base legal art. 7º/11; Súmula 568 TST (antecedentes apenas para funções que justifiquem); Lei 9.029/1995 (proibição de práticas discriminatórias na admissão — exame de gravidez, esterilização).
> - **Non-compete enforceability (state-by-state)** → jurisprudência TST: exige (a) prazo razoável (geralmente até 2 anos), (b) limite geográfico definido, (c) contrapartida financeira durante o período. Sem qualquer um dos três = inexigível. CCT pode regular. Cláusula de não-aliciamento de clientes/empregados mais ampla é tolerada.
> - **I-9 / right-to-work verification** → CTPS digital (Carteira de Trabalho Digital); e-Social S-2200 (admissão) com prazo até o dia anterior ao início da prestação de serviços; estrangeiros: visto/autorização de residência conforme Lei 13.445/2017 + Resolução Normativa CNIg.
> - **EEOC / state agency** → MPT (procedimento preparatório/inquérito civil); TRT; Auditoria-Fiscal do Trabalho.
> - **State** → UF + **categoria sindical + base territorial + CCT/ACT vigente**. Para checagem real, identifique a categoria pela função efetiva (não pelo CBO no contrato) e a base territorial do sindicato.
> Checagens obrigatórias adicionais BR: ASO admissional (NR-7 PCMSO) antes do início; anotação CTPS digital em até 5 dias úteis; contribuição sindical (facultativa pós-Reforma); CIPA (NR-5 conforme grau de risco); eSocial S-2200 e S-2210 (CAT se for o caso).

# /hiring-review

1. Load `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → jurisdictional footprint, hiring review triggers, restrictive covenant policy.
...
`~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → jurisdictional footprint, hiring review triggers, restrictive
...
Check the jurisdiction table in `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` for this state/country. If it's
...
Per `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` restrictive covenant policy: does this hire even get one?
...
Check the `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` table for this jurisdiction. Common categories to
...
**Before producing a "Clear to send" recommendation or a final offer letter for signature:** Read `## Who's using this` in `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md`. If the Role is **Non-lawyer**:
