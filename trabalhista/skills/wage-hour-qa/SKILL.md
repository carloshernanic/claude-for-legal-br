...
name: wage-hour-qa
description: >
  Q&A trabalhista ciente de jurisdição e categoria — classificação CLT/PJ,
  jornada e horas extras (CLT 58–75; CF 7º XVI), intervalos (CLT 71; supressão
  parcial indenizatória pós-Reforma 13.467/2017), afastamentos e licenças
  (Lei 8.213/91; Lei 11.770/2008), verbas rescisórias (CLT 477; Lei
  12.506/2011) — respondido para a UF/categoria específica com a regra
  pesquisada e citada (CLT, Súmulas TST, OJs SDI, Reforma 13.467, CCT/ACT
  vigente) em vez de afirmada de memória. Use quando o(a) usuário(a) fizer
  qualquer pergunta trabalhista, ou disser "qual é a regra em [UF/categoria]",
  "isto se enquadra no art. 62", "temos que pagar HE para", "podemos
...
> - **FLSA exempt/non-exempt + state thresholds** → CLT art. 62 (I — externos sem controle; II — gerentes/diretores com gratificação de função ≥40% sobre cargo efetivo; III — teletrabalho por produção pós-Reforma). Acima do art. 62 → controle de jornada obrigatório se mais de 20 empregados (CLT 74 §2º).
> - **Federal overtime (1.5x) + state overlay (daily OT, double-time)** → adicional **mínimo 50%** (CF 7º XVI) — CCT pode estipular mais (60%, 75%, 100%). Limite **2h/dia** (CLT 59). Banco de horas: acordo individual (compensação em até 6 meses) ou ACT/CCT (até 1 ano) — Reforma 13.467/2017. Trabalho noturno (CLT 73): adicional 20% sobre hora diurna; hora noturna reduzida (52'30"). Rurais: adicional noturno 25% (Lei 5.889/73).
> - **Meal/rest breaks (CA penalty pay)** → intervalo intra-jornada CLT 71: 1h se >6h trabalhadas; 15min se >4h. **Supressão parcial pós-Reforma**: indeniza apenas o tempo suprimido com adicional 50%, natureza **indenizatória** (mudou — antes era pago integralmente + reflexos). Intervalo interjornadas: 11h (CLT 66). Súmula 437 TST foi parcialmente alterada — verificar antes de aplicar a regra antiga.
> - **Final-pay timing (state-by-state)** → CLT 477 §6º: **10 dias corridos**. Atraso → multa art. 477 §8º (1 salário do(a) empregado(a)).
> - **PTO payout** → férias (CF 7º XVII): 30 dias após 12 meses de aquisição + 1/3 constitucional; pagamento até 2 dias antes do início (CLT 145); férias proporcionais na rescisão sem justa causa.
> - **Pay transparency (state laws)** → Lei 14.611/2023; relatórios de transparência salarial e critérios remuneratórios semestrais (MTE — Decreto 11.795/2023).
> - **Classification (ABC / economic realities / common-law)** → requisitos da relação de emprego CLT art. 3º (pessoalidade, não-eventualidade, onerosidade, subordinação); fraude → CLT art. 9º. Pejotização: análise dos requisitos reais, não da forma do contrato. Terceirização: Súmula 331 TST + Lei 13.429/2017 (terceirização irrestrita). Autônomo exclusivo: CLT art. 442-B (pós-Reforma — admitido, com requisitos).
> - **§207(e) regular rate inclusions** → base de cálculo de horas extras = salário base + adicionais habituais (insalubridade, periculosidade, noturno, comissões) + gratificações de natureza salarial. Súmula 264 TST. **Súmula Vinculante 4 STF**: adicional de insalubridade — não pode ter o salário mínimo como base sem lei, mas a base do salário mínimo continua até nova regulamentação (CLT 192).
> - **Liquidated damages (FLSA §216(b))** → não há "liquidated damages" automáticas; juros + correção; multa art. 477 §8º se atraso na rescisão; multa do art. 467 (50% sobre verbas incontroversas se não pagas em audiência).
> - **SOL 2/3 years (FLSA willful)** → **prescrição CF 7º XXIX: 5 anos no curso do contrato; 2 anos a contar da rescisão (bienal final).**
> - **EEOC / DOL** → MPT, Auditoria-Fiscal, TRT/TST.
> - **States** → UFs + **categorias sindicais + CCT/ACT vigente** (a CCT pode dispor diferente da lei nos pontos do CLT art. 611-A — Reforma).
> - Pesquisa de citações: prefira Planalto (texto consolidado de lei), TST (súmulas e OJs), JusBrasil/JusLaboris (julgados), Mediador MTE (CCT/ACT).

# /wage-hour-qa

1. Load `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → jurisdictional footprint.
...
`~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → jurisdictional footprint. If the question doesn't specify a
...
- "Can we classify this person as a contractor?" — Route to
  `/trabalhista:worker-classification` if the facts are not already clear.
