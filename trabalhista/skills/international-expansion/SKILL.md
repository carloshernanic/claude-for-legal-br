...
description: >
  Referência: framework de planejamento para contratação internacional —
  enquadramento da decisão EOR vs. entidade local (subsidiária/filial),
  gatilhos cross-funcionais para tax/finanças/RH-DP, requisições estruturadas
  de briefing para advocacia trabalhista local, e tracker persistente de
  lacunas. Carregada por /expansion-kickoff e /expansion-update; não é
  invocada diretamente.
user-invocable: false
---

> **Adaptação BR:** Skill adaptada para uso a partir de um(a) empregador(a)-base brasileiro(a) que expande para o exterior, OU para empregadora estrangeira contratando no Brasil. Mapeamentos-chave:
> - **EOR vs. entity decision** → mesma estrutura. No Brasil, "EOR" entra como **empregador formal brasileiro contratando para tomadora estrangeira** (CLT vínculo direto com o EOR; tomadora estrangeira ≠ empregadora para fins trabalhistas, mas atenção a grupo econômico CLT 2º §2º e responsabilidade solidária). Entidade local = filial, subsidiária ou EIRELI/Ltda (LC 123/2006 ou Lei 14.195/2021).
> - **Permanent Establishment (PE) risk** → no Brasil, riscos análogos: **caracterização de estabelecimento permanente para fins de IR** (art. 147 do RIR/2018; tratados contra bitributação — modelo OCDE art. 5º); CIDE-remessas; ISS/IRRF na remessa de pagamentos; INSS sobre folha; obrigações acessórias.
> - **PE-risk roles (sales, BD, contracting authority)** → mesma análise no BR; agente que conclui contratos em nome de matriz no exterior pode caracterizar EP (jurisprudência CARF, doutrina).
> - **Mandatory benefits / employer contributions** → no BR: salário mínimo (CF 7º IV) ou piso da categoria (CCT); 13º salário (Lei 4.090/62); férias + 1/3 (CF 7º XVII); FGTS 8% (Lei 8.036/90); INSS patronal ~20% sobre folha + SAT 1-3% + sistema S; vale-transporte (Lei 7.418/85); ASO admissional/periódico (NR-7); PPP/LTCAT conforme atividade.
> - **Termination protections (notice + severance)** → CLT 477 + Lei 12.506/2011 (aviso prévio proporcional) + FGTS+40% + verbas integrais; estabilidades (ver CLAUDE.md).
> - **Restrictive covenants (non-compete enforceability)** → jurisprudência TST exige prazo + geografia + contrapartida.
> - **Works council / collective bargaining (EU/Germany)** → no BR: **CCT/ACT** (negociação coletiva — CF 7º XXVI; CLT arts. 611–625); **comissão de representantes dos empregados** em empresa com mais de 200 empregados (CLT arts. 510-A–510-E, Reforma 13.467/2017); CIPA (NR-5); dirigente sindical (CF 8º VIII).
> - **Data protection (GDPR)** → **LGPD** (Lei 13.709/2018) — análoga ao GDPR; transferência internacional de dados de empregado(a)s (arts. 33–36 LGPD); ANPD regula.
> - **Work authorization** → para estrangeiros no BR: visto/autorização de residência (Lei 13.445/2017 — Lei de Migração); Resoluções Normativas do CNIg; carteira de trabalho digital; CPF; eSocial.
> - **Equity / RSU tax (employee + employer side)** → no BR: stock options têm tratamento controvertido na jurisprudência TST (Tema repetitivo — natureza pode ser mercantil ou salarial conforme habitualidade e onerosidade); IRPF na recompra/exercício; IN RFB 1.500/2014.
> - **Outside counsel briefing (every country gets one)** → mantém-se. Para o Brasil, briefing deve cobrir: CLT, Reforma 13.467/2017, jurisprudência TST corrente (atenção: várias súmulas alteradas pela Reforma — checar), CCT/ACT da categoria, eSocial, LGPD, NRs do MTE aplicáveis ao setor.
> - **Contractor / IC risk** → pejotização; análise CLT 3º; Súmula 331 TST (terceirização); Lei 6.019/74 (temporário); MEI exclusivo (alerta).
> - **Day 1 compliance (BR)** → CTPS digital + S-2200 e-Social no dia anterior à admissão; ASO admissional; PCMSO/PGR; vale-transporte; CIPA (se aplicável); cadastro PIS/NIT; CCT/ACT vigente carregada.

# Skill de Expansão Internacional
...
Read `~/.claude/plugins/config/claude-for-legal/trabalhista/CLAUDE.md` → jurisdictional footprint, escalation table, any existing
expansion notes.
...
Write a new file to `~/.claude/plugins/config/claude-for-legal/trabalhista/expansion-[country-slug].yaml` with all open items
...
**Roles:** [list]
**Tracker:** ~/.claude/plugins/config/claude-for-legal/trabalhista/expansion-[slug].yaml
...
Run `/trabalhista:expansion-update [country]` to update status
as items close.
```
