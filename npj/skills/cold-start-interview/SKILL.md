...
description: >
  Setup único do(a) professor(a) do NPJ — áreas de prática, jurisdição (comarca,
  foros, justiça especializada), modelo de supervisão (fila formal / flags
  configuráveis / mão leve) e upload de regimento/normas. Escreve CLAUDE.md
  para que toda skill e todo(a) estagiário(a) que rode /ramp leia o mesmo
  contexto do NPJ. Use em instalação nova, quando CLAUDE.md tem placeholders,
  ao refazer setup com --redo, ou ao re-checar integrações com --check-integrations.
argument-hint: "[--redo] [--check-integrations]"
---

# /cold-start-interview

> **Adaptação BR (não oficial).** Setup voltado ao(à) professor(a) supervisor(a) advogado(a) OAB do NPJ. Captura: papel (professor(a) OAB obrigatório; estagiário(a) é roteado para `/ramp`); seccional e número de inscrição OAB; pré-condições do Provimento CFOAB 11/2007 e Lei 11.788/2008 (estágio); Resolução CNE/CES 5/2018 (NPJ); áreas (família/CC + Maria da Penha 11.340/2006 + ECA + Alienação Parental 12.318/2010; inquilinato/Lei 8.245; consumidor/CDC + superendividamento 14.181/2021; trabalhista/CLT; previdenciário/Lei 8.213 + BPC LC 142/2013; criminal/reabilitação CP 93-95 + Tema 786 STF; migração-refúgio/13.445/2017 + 9.474/97 + CONARE); comarca, vara, JEC (Lei 9.099 — 20 SM), JEF (10.259), Fazenda (12.153), TRT, Vara da Infância, Justiça Federal; modelo de supervisão; documentos-semente (regimento do NPJ, manual do tribunal — PJe/e-SAJ/e-Proc/Projudi, fichas de atendimento). Encaminhamentos: Defensoria (LC 80/94), Procon, CRAS/CREAS, DEAM, Conselho Tutelar, MP, CONARE, CEJUSCs (Lei 13.140/2015). Sigilo: EOAB art. 7º XIX.

1. Check `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md`. If populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.claude/plugins/cache/claude-for-legal/npj/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` including `## Who's using this` and `## Available integrations`. Show supervision choice and practice-area templates for confirmation.
7. Offer `/npj:ramp` preview.

```
/npj:cold-start-interview
```

**`--check-integrations`:** Re-run only the Part 0 integration-availability check (Clio, document storage). Updates `## Available integrations` in `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` without touching the role, ethical preconditions, supervision style, or practice-area templates. Use after adding or removing an MCP connector.
...
Read `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md`:
...
> **`npj` is for supervising attorneys setting up a law school clinic and onboarding students.** Not your area? `/builder-hub:related-skills-surfacer`.
>
...
- **Pause and resume.** Tell the supervising attorney up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/npj:cold-start-interview` again later and I'll pick up where you left off." When the attorney pauses, write a partial configuration to `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run /npj:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the attorney: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> 2. **No, I'm a student / staff / administrator.** Stop. This setup writes the clinic's governing context — supervision model, client-data rules, ethical preconditions — and must be done by the supervising attorney who will be accountable for the work. Ask them to run `/npj:cold-start-interview`. Students run `/npj:ramp` to onboard each semester.
...
Write Part 0 answers to the plugin config under `## Who's using this` and `## Available integrations`. If a populated CLAUDE.md exists at the old cache path `~/.claude/plugins/cache/claude-for-legal/npj/*/CLAUDE.md` but not here, copy it forward first.
...
>
> You can set this per document type later with `/npj:build-guide`. For now, pick a default.
...
> Do you want to author a practice-area guide that tailors how the skills work for your clinic — intake questions, per-document pedagogy overrides, review gates? I can help you build one in 5-10 minutes with `/npj:build-guide`. You can also do it later. For now, the skills use sensible defaults: the pedagogy default you just picked, and everything client-facing flagged for your review.

Note the answer in the setup state — if the supervisor wants to build a guide, surface that as a next step after the interview closes (under Step 3 of the "After writing" section). Do not interrupt this interview to run `/npj:build-guide` inline; finish the profile first, then offer the handoff.
...
**LIMITED DATA flag:** if fewer than 10 materials were shared across the interview, add a `> LIMITED DATA` note at the top of CLAUDE.md (under the written-on date), stating: "This practice profile was written from [N] materials. Downstream skills will operate but outputs will be thinner — `/ramp` covers commands but not clinic-specific procedures, `/draft` uses state defaults instead of local formatting, `/client-letter` uses generic templates. Re-run `/npj:cold-start-interview --redo` after collecting more exemplars to sharpen calibration."
...
>
> - **Student intake on a new case** — e.g., "Walk a student through a practice-area-specific intake with red-flag spotting and conflict checks." Try: `/npj:client-intake`
> - **Draft a client letter at 6th-grade reading level** — e.g., "Produce an appointment confirm or status update in plain language; student edits and you approve." Try: `/npj:client-letter`
> - **Build an IRAC memo scaffold** — e.g., "Give a student the structure and research-gap list for a case memo — pedagogy default is guide." Try: `/npj:memo`
> - **Track deadlines across the active docket** — e.g., "See what's due in the next 14 / 7 / 3 / 1 days with warnings per your cadence." Try: `/npj:deadlines`
> - **Ramp up a new cohort** — e.g., "Onboard this semester's students to the clinic's procedures, tools, and case-handling norms." Try: `/npj:ramp`
> - **Semester handoff** — e.g., "Build per-case transition memos for the incoming cohort." Try: `/npj:semester-handoff`
>
...
> Done. Your clinic's configuration is at `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
> - Edit the file directly for a quick change
> - Run `/npj:cold-start-interview --redo` for a full re-interview
> - Run `/npj:cold-start-interview --check-integrations` to re-check what's connected
>
