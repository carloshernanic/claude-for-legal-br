...
name: ramp
description: >
  Onboarding do(a) estagiário(a) do NPJ no semestre — procedimentos do NPJ,
  passada pelas ferramentas, exercícios práticos antes dos casos reais. Lê o
  regimento que o(a) professor(a) subiu no setup e ensina interativamente. Use
  quando estagiário(a) novo(a) diz "me dê onboarding", "sou novo(a) no NPJ",
  "começando", ou no início de cada semestre; passe --card para a referência
  de uma página.
argument-hint: "[--card para a referência de uma página]"
---

> **Adaptação BR (não oficial).** Skill para onboarding do(a) estagiário(a) de NPJ. Base normativa do estágio: Estatuto OAB (Lei 8.906/94) art. 3º §2º (estagiário(a) inscrito(a) atua junto com advogado(a); pode subscrever petições e praticar atos próprios da advocacia em conjunto); Provimento CFOAB 11/2007 e seguintes; Lei 11.788/2008 (estágio); Resolução CNE/CES 5/2018 (Diretrizes Curriculares do Direito — NPJ obrigatório). Ética: CED-OAB (Código de Ética e Disciplina), Provimento CFOAB 205/2021 (publicidade) e Provimento CFOAB 218/2023 (uso de IA na advocacia). Conflito de interesses: EOAB 17-19. Sigilo profissional: EOAB 7º XIX, CPC 388. Tribunais comuns: PJe (TST/TRTs, vários TJs), e-SAJ (TJSP entre outros), e-Proc (TRFs e alguns TJs), Projudi (alguns TJs). Conduta junto ao(à) assistido(a): identificação como estagiário(a), papel auxiliar, professor(a) responsável, gratuidade (CPC 98-102; hipossuficiência declarada CPC 99 §3º). Áreas comuns do NPJ: família (Maria da Penha 11.340/2006 atravessa), inquilinato (8.245), consumidor (CDC + 14.181/2021), trabalhista (CLT — jus postulandi até 2 SM CLT 791), previdenciário (8.213 + BPC LC 142/2013), criminal, migração/refúgio (13.445/2017 + 9.474/97 + CONARE).

# /ramp

1. Check `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` is set up. If placeholders: "Ask [professor] to run `/npj:cold-start-interview` first."
...
`~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → clinic profile, practice areas, jurisdiction, handbook path, supervision style, practice-area templates.
...
Read from `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` and the ingested handbook. Cover, interactively:
