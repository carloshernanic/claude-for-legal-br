...
name: research-start
description: >
  Roadmap de pesquisa para questão jurídica — leis a conferir, áreas de
  jurisprudência a investigar, frameworks regulatórios, termos de busca em
  JusBrasil/STJ/STF/TST/TRT/Planalto/LEXML. Pistas e frameworks, NÃO citações
  autoritativas; estagiário(a) verifica e desenvolve tudo. Use quando
  estagiário(a) pergunta por onde começar pesquisa, quer roadmap para uma
  questão, ou precisa identificar lacunas em pesquisa existente.
argument-hint: "[questão jurídica]"
---

> **Adaptação BR (não oficial).** Skill para NPJ — roadmap de pesquisa no sistema civil-law brasileiro. Pirâmide normativa para investigar: CF/88, leis complementares, leis ordinárias, decretos, instruções normativas, súmulas vinculantes (CF 103-A), súmulas STF/STJ/TST, precedentes qualificados CPC 927 (Temas de Repetitivos, Repercussão Geral, IRDR, IAC), jurisprudência persuasiva, doutrina. Fontes esperadas: **Planalto** (planalto.gov.br — textos consolidados), **LEXML** (lexml.gov.br — bases legais), **JusBrasil** (jurisprudência e doutrina), **STF/STJ/TST/TRTs/TJs** (sites oficiais com inteiro teor e acórdãos), **Vade Mecum** (Saraiva/Atlas/etc.), **diários oficiais** (DOU, DJE estadual). Para temas específicos: INSS (Lei 8.213 + IN; gov.br/inss), Procon (sites estaduais; Senacon/gov.br/senacon), CONARE (gov.br/mj/conare), MJ, MTE/MTb, ANPD (LGPD), TCU. Atualidade obrigatória: reforma trabalhista (Lei 13.467/2017), reforma do CPC, Marco Civil/LGPD/LIA reformada, Lei 14.181/2021 (superendividamento), Temas pendentes. Saída sinaliza tudo como `[conhecimento do modelo — verificar]` ou `[recuperado mas verificar sustentação]` — estagiário(a) confere. Etiqueta `[RASCUNHO ASSISTIDO POR IA — exige análise do(a) estagiário(a) e revisão do(a) professor(a) advogado(a) OAB]`.

# /research-start

1. Load `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → jurisdiction, practice area.
...
```
/npj:research-start "habitability defense to nonpayment eviction in [State]"
```
...
`~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → jurisdiction (state), practice areas.
...
1. Read `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → `## Seed documents`. Identify any item whose purpose or filename matches the research area (e.g., "Alameda UD filing guide" for a UD habitability question; a redacted sample case file in the same practice area; a prior memo on the same issue).
