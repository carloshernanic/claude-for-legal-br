...
description: >
  Registra comunicação com assistido(a) — ligação, e-mail, WhatsApp, carta,
  presencial, mensagem de voz. Registro append-only por caso com data, direção,
  meio, resumo, providências. Trabalha junto com /client-letter e /status client.
  Use ao registrar ligação ou e-mail de assistido(a), revisar log de comunicação,
  ou perguntar "o que dissemos ao(à) [assistido(a)] da última vez".
argument-hint: "[id-caso] [--add (default) | --read | --summary | --patterns]"
---

> **Adaptação BR (não oficial).** Skill para NPJ. Substitua "client" por "assistido(a)" do NPJ. Sigilo: EOAB art. 7º XIX, CPC 388. Registro de comunicação alimenta a supervisão do(a) professor(a) advogado(a) OAB (Provimento CFOAB 11/2007; Resolução CNE/CES 5/2018). LGPD (Lei 13.709/2018): dados pessoais do(a) assistido(a) tratados com base legal art. 7º IX + minimização. Cuidado especial em casos sob segredo de justiça (CPC 189: família, infância, jurisdição voluntária, dados sensíveis; ECA 143; Lei Maria da Penha; Lei 9.474/97 art. 23 refúgio). WhatsApp/SMS como meio é comum no atendimento brasileiro — registre número, mas evite encaminhar dados sensíveis por canal não-criptografado fora do círculo de sigilo.
...
3. Route by flag:
   - `--add` (default): capture direction, medium, student, summary, action items, follow-up due. Confirm with user. Append (prepend most-recent-first) to `~/.claude/plugins/config/claude-for-legal/npj/client-comms/[case-id]/log.md`.
...
4. Integration: offer `/npj:deadlines --add` if the log establishes a deadline; route to `/npj:semester-handoff` via `--summary` when relevant.
...
- `~/.claude/plugins/config/claude-for-legal/npj/client-comms/[case-id]/log.md` (if exists) — append target
- `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → not heavily read; this skill is case-scoped
...
**Append** to `~/.claude/plugins/config/claude-for-legal/npj/client-comms/[case-id]/log.md`. If the log doesn't exist, create it with a header:
