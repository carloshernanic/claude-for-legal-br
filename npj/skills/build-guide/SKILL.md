...
name: build-guide
description: >
  Ajuda professor(a) supervisor(a) do NPJ a escrever guia por área de prática que
  configura como as skills voltadas a estagiários(as) se comportam — perguntas de
  atendimento, postura pedagógica (assist / guide / teach), gates de revisão,
  checagens cross-plugin e regras locais. Use quando professor(a) advogado(a) OAB
  quer construir ou revisar guia por área, calibrar como as skills clínicas se
  comportam para o tipo do NPJ, ou setar a filosofia pedagógica como configuração
  do plugin.
argument-hint: "[opcional: área — ex. 'familia', 'consumidor', 'inquilinato', 'refugio']"
---

> **Adaptação BR (não oficial).** Skill voltada ao(à) professor(a) supervisor(a) de Núcleo de Prática Jurídica (NPJ). Base: Resolução CNE/CES 5/2018, Estatuto OAB (Lei 8.906/94) art. 3º §2º, Provimento CFOAB 11/2007, Lei 11.788/2008 (estágio). Áreas tipicamente cobertas: família (CC + Lei Maria da Penha 11.340/2006 + ECA + Lei 12.318/2010 Alienação Parental), inquilinato (Lei 8.245/91), consumidor (CDC + superendividamento Lei 14.181/2021), trabalhista (CLT + TRT), previdenciário (Lei 8.213/91 + BPC/LOAS LC 142/2013), criminal (defesa + reabilitação CP 93-95 + Tema 786 STF + dativo CPP 263), migração/refúgio (Lei 13.445/2017 + Lei 9.474/97 + CONARE). Encaminhamentos: Defensoria Pública (LC 80/94), Procon, CRAS/CREAS, DEAM, Conselho Tutelar, MP, CEJUSCs (Lei 13.140/2015 mediação). Sigilo: EOAB art. 7º XIX. Substitua referências US (Legal Aid → Defensoria + NPJ; pro bono → Provimento CFOAB 166/2015; pro se → CPC 103 + JEC 20 SM Lei 9.099 / HC CPP 654 / trabalhista 2 SM CLT 791; eviction → ação de despejo; expungement → reabilitação; bankruptcy → Lei 14.181/2021 e 11.101/2005).

# /build-guide

1. Load `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → role (must be Supervising attorney), practice areas, jurisdiction.
2. Use the workflow below.
3. If the user is not the supervising attorney, stop and redirect (students run `/npj:ramp`).
4. Walk through: practice area → intake questions → pedagogy posture → review gates → cross-plugin checks → local rules.
5. Write `~/.claude/plugins/config/claude-for-legal/npj/guides/<practice-area>.md`. Create the `guides/` directory if needed.
6. Offer a test run — run `/npj:draft` under the configured posture so the supervisor sees what a student sees.
...
**Audience: the supervising attorney.** Not students. Students run `/npj:ramp` and then the student-facing skills; they don't author guides.
...
This is a supervisor skill. Read `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → `## Who's using this` → Role. If the role is not "Supervising attorney," say:

> This skill is for supervisors — it configures how the student-facing skills behave. If you're the supervisor, make sure your practice profile role is set to "Supervising attorney" in `/npj:cold-start-interview`. If you're a student, this isn't the right skill for you — run `/npj:ramp` to onboard, or ask your supervisor to author a guide for your clinic.
...
Check the practice areas listed in `CLAUDE.md` → `## Clinic profile` → Practice areas. If the chosen practice area is not listed there, note it: "I'll write this guide, but your practice profile doesn't list [area] as one of your clinic's practice areas. That's fine — you can add it later with `/npj:cold-start-interview --redo` — but the student-facing skills won't route intakes to this area until the profile lists it."

If a guide already exists at `~/.claude/plugins/config/claude-for-legal/npj/guides/<practice-area>.md`, offer: "A guide for [area] already exists at [path]. Do you want to (a) revise it section-by-section, (b) start fresh and overwrite, or (c) see what's there first?"
...
- **Transactional clinic:** `comercial:review` (NDA triage, vendor review) wrapped so the student runs the review, the output is flagged for supervisor review before going to the client.
- **Immigration clinic:** `contencioso:chronology` for building a timeline from client documents, flagged for supervisor review before it feeds a filing.
- **Housing clinic:** `contencioso:subpoena-triage` when the client brings in a subpoena, wrapped so the student drafts the response plan but the supervisor signs off.
- **Any clinic:** `privacidade:triage` if the student is handling any matter where personal data is shared outside the clinic.
...
Write to `~/.claude/plugins/config/claude-for-legal/npj/guides/<practice-area>.md`. Create the `guides/` directory if it doesn't exist. Use this structure:

```markdown
# Practice-area guide: [Practice area]

*Authored by the supervising attorney via `/npj:build-guide`. Student-facing skills read this before producing output. Edit directly anytime.*
...
> Your guide is at `~/.claude/plugins/config/claude-for-legal/npj/guides/<practice-area>.md`. Every student who uses the clinic plugin for [practice area] will have skills that follow it. Edit the file directly to change anything, or re-run `/npj:build-guide` to revise a section. You can have multiple guides — one per practice area.

### Step 9: Offer a test run

> Want to see how the pedagogy posture changes the experience? I'll run `/npj:draft` with a sample client letter under [posture] — you'll see what the student sees.
...
The skill's "output" is the file written at `~/.claude/plugins/config/claude-for-legal/npj/guides/<practice-area>.md`. The conversation with the supervisor is the interview; the written guide is the artifact.
...
>
> Students will see these changes the next time they run a clinic command for this practice area. Edit `[path]` anytime to change anything, or re-run `/npj:build-guide` to revise.

## What this skill does NOT do

- **Configure the plugin globally.** The guide is per-practice-area. For plugin-wide config (supervision style, jurisdiction, practice areas), that's `/npj:cold-start-interview`.
