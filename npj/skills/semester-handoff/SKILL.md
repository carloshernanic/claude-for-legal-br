...
description: >
  Memorandos de passagem de caso ao fim do semestre — o espelho de /ramp.
  Produz memorandos de transição por caso e um resumo da turma para que a
  turma que sai entregue o trabalho à turma que entra de modo limpo. Lê
  prazos, comunicações com assistido(a) e histórico do caso. Use quando
  professor(a) ou estagiários(as) que saem precisam encerrar o semestre,
  montar memorandos de transição, ou fazer offboarding de estagiário(a)
  que se forma ou desliga.
argument-hint: "[--semestre=AAAA-periodo (default: atual)] [--caso=[id_caso] (para um caso só)]"
---

> **Adaptação BR (não oficial).** Skill para passagem de semestre no NPJ. Considere o calendário acadêmico brasileiro (semestres tipicamente fev-jul e ago-dez) versus prazos forenses (recesso CPC 220 — 20/dez a 20/jan suspende prazos cíveis; recesso trabalhista; prazos penais correm em dias corridos). Sigilo: EOAB art. 7º XIX, CPC 388 — estagiário(a) que sai mantém dever de sigilo perpétuo sobre os casos. Provimento CFOAB 11/2007 e Lei 11.788/2008 (estágio): comunicar saída ao(à) professor(a); atualizar registro do NPJ; arquivo dos casos transferidos com cadeia de custódia. LGPD (Lei 13.709/2018): cuidado com dados pessoais do(a) assistido(a) ao transferir entre estagiários(as) — minimização; controle de acesso. Casos sob segredo de justiça (CPC 189, ECA 143, Maria da Penha, refúgio Lei 9.474/97 art. 23) exigem cuidado adicional na transição. Encaminhamentos pendentes para Defensoria/Procon/CRAS/CREAS/DEAM/MP devem aparecer no memorando.

# /semester-handoff

1. Load `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → clinic profile, semester dates, supervision style.
2. Load `~/.claude/plugins/config/claude-for-legal/npj/deadlines.yaml` and `~/.claude/plugins/config/claude-for-legal/npj/client-comms/[case-id]/log.md` per case.
...
5. Generate per-case handoff memo → `~/.claude/plugins/config/claude-for-legal/npj/handoffs/[semester]/[case_id].md`.
6. Generate cohort summary → `~/.claude/plugins/config/claude-for-legal/npj/handoffs/[semester]/_summary.md`.
...
- `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → clinic profile, semester, practice areas, supervision style
- `~/.claude/plugins/config/claude-for-legal/npj/deadlines.yaml` → all active deadlines, grouped by case
- `~/.claude/plugins/config/claude-for-legal/npj/client-comms/[case-id]/log.md` (per case) → communications history
...
- Pull all active cases (from intake records + `~/.claude/plugins/config/claude-for-legal/npj/deadlines.yaml` case_ids + client-comms folders)
...
*Pulled from `~/.claude/plugins/config/claude-for-legal/npj/deadlines.yaml`. Incoming student's first job is to confirm these are accurate and owned.*
...
*From `~/.claude/plugins/config/claude-for-legal/npj/client-comms/[case-id]/log.md`. Three-line summary here; incoming student reads the full log.*
...
After all per-case memos, produce `~/.claude/plugins/config/claude-for-legal/npj/handoffs/[semester]/_summary.md`:
...
Once reviewed, handoff memos live at `~/.claude/plugins/config/claude-for-legal/npj/handoffs/[semester]/[case_id].md`. The incoming student reads them during their `/ramp` run at the start of next semester — `/ramp` should surface the memos for cases the new student is assigned.

## Integration

- **`/ramp`:** at the start of next semester, reads `~/.claude/plugins/config/claude-for-legal/npj/handoffs/[most-recent-semester]/` and surfaces per-case memos for the cases each new student is taking on.
...
- **Close cases.** Handoff is for cases transitioning to the next cohort. Cases closing at semester end should get a final internal status memo (`/npj:status internal`) for the file and be marked closed in the handoff document; the status skill supports `client | internal | court` audiences.
