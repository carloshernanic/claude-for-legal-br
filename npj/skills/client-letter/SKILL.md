...
name: client-letter
description: >
  Correspondência de rotina com assistido(a) a partir de templates — confirmação
  de agendamento de atendimento, solicitação de documentos, atualização breve
  do tipo "protocolamos a petição". Linguagem simples, elementos obrigatórios,
  roteamento de supervisão. NÃO é orientação substantiva. Use quando estagiário(a)
  precisa enviar correspondência de rotina, confirmação de agenda, pedido de
  documentos ou nota curta de status ao(à) assistido(a).
argument-hint: "[atendimento | pedido-doc | atualizacao]"
---

> **Adaptação BR (não oficial).** Skill para NPJ — comunicação com assistido(a) sob supervisão do(a) professor(a) advogado(a) OAB. Linguagem simples (ensino fundamental II como nível alvo), sem juridiquês ("outrossim", "destarte", "consoante", "vossa excelência" em carta ao(à) assistido(a) são banidos). Elementos obrigatórios: o que aconteceu, o que vem a seguir, o que o(a) assistido(a) precisa fazer, como falar com o NPJ (telefone, endereço, horário de atendimento), prazo se houver. Sigilo: EOAB art. 7º XIX. Verifique destino antes de enviar — WhatsApp pessoal do(a) assistido(a) está dentro do círculo; grupo amplo não. Encaminhamentos comuns mencionados em cartas: Defensoria Pública (LC 80/94) para representação plena; Procon (relação de consumo extrajudicial); CRAS/CREAS (vulnerabilidade social); DEAM (violência doméstica — Lei Maria da Penha); Conselho Tutelar (criança/adolescente). LGPD: minimize dados em correspondência.

# /client-letter

1. Load `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → plain-language standards, supervision style, clinic contact info.
...
```
/npj:client-letter appointment
```

```
/npj:client-letter doc-request
```
...
`~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → plain-language standards, supervision style, clinic contact info.
...
Sending a letter to a client is a consequential action. This plugin's gate is the supervision workflow described in `## Supervision style` in `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md`, reinforced by the Part 0 role check that confirms a licensed supervising attorney owns the clinic setup. That gate still holds: every letter clears review before it leaves the clinic.

Before sending any of the letters above, confirm:

1. The draft has been reviewed per the supervision protocol in `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` (queue / flag / lighter-touch).
...
3. The sign-off conforms to your jurisdiction's student practice rule for estudante-direito-signed correspondence.
...
Per `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` standards. Short sentences. No jargon. Reading level target enforced. If a template above includes a legal term the client might not know, explain it the first time: "We filed your 'answer' — that's the document that tells the court your side of the story."

## Supervision routing

Per `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md`. Routine correspondence may or may not be a flag trigger depending on the supervision style the professor chose. If lighter-touch: these go out after student review without a queue step. If formal queue: even routine letters queue.
