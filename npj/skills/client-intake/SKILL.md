...
name: client-intake
description: >
  Atendimento estruturado de NPJ — templates por área, identificação de issues
  cross-área, flags de conflito e classificação de triagem. Produz ficha de
  atendimento formatada que o(a) estagiário(a) analisa e o(a) professor(a) revisa.
  NÃO decide aceitação do caso. Use ao iniciar atendimento de novo(a) assistido(a),
  conduzir entrevista, ou redigir a situação do(a) assistido(a) novo(a).
argument-hint: "[opcional: dica de área — família, consumidor, inquilinato, etc.]"
---

> **Adaptação BR (não oficial).** Skill para NPJ — atendimento inicial sob supervisão do(a) professor(a) advogado(a) OAB. Substitua US-centric por: assistido(a) (não "client" comercial); hipossuficiência declarada (CPC 99 §3º) + gratuidade da justiça (CPC 98-102); encaminhamento à Defensoria Pública (LC 80/94) quando o caso fugir da capacidade do NPJ. Templates por área esperados:
> - **Família** → CC 1.511-1.783; Maria da Penha 11.340/2006 (medidas protetivas; DEAM); Alienação Parental 12.318/2010; ECA 8.069/90; alimentos provisórios; guarda; divórcio.
> - **Inquilinato** → Lei 8.245/91 (despejo, retomada, ação revisional, consignação em pagamento de aluguel).
> - **Consumidor** → CDC 8.078/90; superendividamento Lei 14.181/2021 (repactuação, mínimo existencial); Procon; JEC (Lei 9.099/95, até 20 SM); ACP.
> - **Trabalhista** → CLT; reclamação no TRT; jus postulandi até 2 SM (CLT 791) — acima exige advogado(a); rescisão indireta CLT 483.
> - **Previdenciário** → Lei 8.213/91 (INSS); BPC/LOAS LC 142/2013; Justiça Federal; JEF (Lei 10.259/2001).
> - **Criminal** → defesa (com Defensoria/dativo CPP 263 sendo paralelos); reabilitação CP 93-95; Tema 786 STF (presunção de inocência e antecedentes); HC (CPP 654 dispensa advogado(a)).
> - **Migração/Refúgio** → Lei 13.445/2017; Lei 9.474/97; CONARE; segredo de justiça.
> - **PCD** → LBI Lei 13.146/2015. **Idoso(a)** → Estatuto Lei 10.741/2003.
> - **LGBTQIA+** → ADO 26 STF (criminalização da LGBTfobia).
> Conflito de interesses: EOAB art. 17-19, CED-OAB. Sigilo: EOAB art. 7º XIX. Tags de fonte: [JusBrasil], [STJ], [STF], [TST], [Planalto], [LEXML], [Defensoria], [Procon], [DEAM], [CREAS].

# /client-intake

1. Load `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → practice areas, intake templates, supervision style, flag triggers.
...
`~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` → practice areas, intake templates (per practice area if multiple), supervision style, jurisdiction, flag triggers.

## Read the supervisor guide

Check for a practice-area guide at `~/.claude/plugins/config/claude-for-legal/npj/guides/<practice-area>.md`. If one exists, use its intake questions, red flags, and good-fit criteria instead of the generic defaults below. If one doesn't exist, use the generic intake and note at the end of the intake summary: "This was a generic intake — your supervisor can tailor the questions for your clinic type with `/npj:build-guide`."
...
Each practice area asks different questions. Use the template from `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` for this area. Defaults if none provided:
...
Per whatever conflict-check process `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` describes. At minimum:
...
Per `~/.claude/plugins/config/claude-for-legal/npj/CLAUDE.md` supervision style and flag triggers. If formal queue or configurable flags are enabled, and a trigger is present (deadline mentioned, DV indicator, immigration status at issue, etc.), note the flag.

### Step 7: Deadline handoff — required deliverable

If the intake surfaces any timeline deadline (answer due, hearing, statute-of-limitations cutoff, cure period, filing window, notice window, ICE check-in, removal hearing, eviction court date, protective order renewal), **emit a copy-paste-ready `/npj:deadlines --add ...` block as part of the intake output**. This is a required deliverable, not a suggestion — the intake identifies deadlines, and the student shouldn't have to re-transcribe them into the deadline skill.
...
[One `/npj:deadlines --add ...` block per surfaced deadline — Step 7.
