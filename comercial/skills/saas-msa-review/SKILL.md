...
description: >
  ...
  /comercial:review when a SaaS or subscription agreement is detected.
user-invocable: false
---

# SaaS / Subscription Agreement Review

**Adaptação BR.** Plugin adaptado ao Brasil. Limitação de responsabilidade em B2B segue CC arts. 421/422/944 (admitida salvo dolo); em consumo (CDC art. 51, I) qualquer atenuação é nula. Renovação automática em adesão (CC art. 423) e consumo (CDC arts. 51, 54) tem leitura restritiva — janela de notificação razoável. LGPD (Lei 13.709/2018) substitui referências a GDPR; comunicação de incidente segue Resolução CD/ANPD 15/2024. Indenização limita-se à extensão do dano (CC art. 944). Cláusula penal: CC arts. 408–416 (cap = valor da obrigação principal). Antes de tudo: regime B2B ou consumo?
...
It runs the standard playbook check from `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md` and adds a SaaS-specific overlay on the terms that bite hardest in subscription deals.

## Premissa jurisdicional

Termos SaaS (notificação de renovação automática, limites de reajuste, portabilidade de dados, regras de subprocessador) são sensíveis à jurisdição. No Brasil: renovação automática em adesão tem leitura restritiva (CC art. 423); em consumo, cláusulas onerosas exigem destaque (CDC art. 54 §4º) e cláusulas do art. 51 são nulas. Reajuste de preço segue Lei 10.192/2001 (proibição de indexação a moeda estrangeira em contrato interno e periodicidade anual em geral). Portabilidade e exclusão de dados pessoais seguem LGPD arts. 18 (direitos do titular) e 16 (eliminação). Subprocessadores e transferência internacional: LGPD arts. 33–36 e Resolução CD/ANPD 19/2024. Esta revisão aplica as posições do `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md`, que assumem lei brasileira. Se o contrato escolhe outra lei (LINDB art. 9º — eficácia limitada em contrato interno), flague.
...
Read `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md` first. The general playbook for the matching side (liability, indemnity, termination, governing law) applies fully — run all the standard checks from the vendor-agreement-review skill.
...
If `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md` doesn't address a SaaS-specific term that comes up in this review, ask:

> Your playbook doesn't cover [term — e.g., "maximum acceptable auto-renewal notice window" or "whether vendor retention of anonymized derivatives is acceptable"]. What's your team's position? I'll add it to `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md`.

Record the answer and proceed.

## SaaS-specific overlay

For each category below, list what you found in the contract and compare to the team's position in `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md`. Do not apply hardcoded thresholds from this skill.
...
Check each element and compare against the team's `SaaS positions` in `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md`:
...
Check each element against `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md`:
...
When (not if) we leave this vendor, can we get our data out? Check each element against `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md`:
...
Vendor retention of "anonymized" or "aggregated" derivatives is a material position — confirm the team's stance in `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md` and flag either way.
...
3. **Padrão de anonimização.** Se o fornecedor afirma que só treina em dado "anonimizado" ou "agregado", qual é o padrão? "Anonimizado" sem definição é frágil. Atende LGPD art. 5º, III (dado anonimizado: não identifica nem é razoavelmente identificável)? É reversível? Lembrar: pseudonimização (art. 13 §4º) ≠ anonimização. Se reversível, continua dado pessoal e LGPD se aplica integralmente.
...
7. **Cadeia regulatória downstream.** O uso de seus dados pelo fornecedor para IA cria exposição regulatória para VOCÊ? Obrigações de controlador LGPD (art. 42), responsabilidade por escolha de operador (art. 39), DPIA (RIPD) quando aplicável, dever de informar titular (art. 9º). Em consumo, prática enganosa (CDC art. 37) por compartilhamento não divulgado. Atenção ao PL 2.338/2023 (marco legal de IA, em tramitação) `[verificar status]` e Resoluções da ANPD sobre IA.
...
**O playbook aplica preferência global de lei. Exequibilidade varia materialmente.** Cheque a lei aplicável do contrato contra as principais divergências antes de aceitar posições do playbook ao pé da letra:

- **Não-aliciamento / não-concorrência:** No Brasil em B2B, válidas se limitadas em tempo, território e atividade, com indenização adequada (CC art. 421/422); pós-CLT exige indenização (TST jurisprudência). Em consumo (B2C), discutíveis. `[verificar jurisdição]`
- **Renovação automática:** CC art. 423 (adesão — leitura favorável ao aderente). CDC arts. 51 e 54 (consumo — onerosa exige destaque, sob pena de nulidade). `[verificar jurisdição]`
- **Exclusão de responsabilidade:** Em B2B paritário admite-se salvo dolo (CC arts. 421/422/944). Em consumo, CDC art. 51, I — qualquer exoneração/atenuação do fornecedor é nula. Em adesão B2B (CC art. 424), renúncia antecipada a direito da natureza do negócio é nula. `[verificar jurisdição]`
- **Indenização:** Limita-se à extensão do dano (CC art. 944). Cláusula penal: cap = valor da obrigação principal (CC art. 412). Em consumo, multa moratória ao consumidor não pode exceder 2% (CDC art. 52 §1º). `[verificar jurisdição]`
- **Prazo de confidencialidade:** "Perpétuo" pode ser reduzido por razoabilidade (CC art. 421). Para segredo de empresa, Lei 9.279/1996 (LPI) art. 195, XI–XII traz proteção legal independente. `[verificar jurisdição]`

Quando a posição do playbook conflita com a exequibilidade sob a lei aplicável, flague: "Seu playbook prefere [X], mas este contrato é regido por [Y] onde [X] é [inexequível / restrito / sujeito a override estatutário]. `[verificar jurisdição]`"
...
### Price escalation
[findings against `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md` positions]
