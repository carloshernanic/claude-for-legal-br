...
**Adaptação BR.** Plugin adaptado ao Brasil. Renovação automática em contratos de adesão tem leitura restritiva (CC art. 423; em consumo, CDC arts. 51 e 54). Notificação por meio eletrônico tem força conforme MP 2.200-2/2001 e Lei 14.063/2020. Prazos contam-se em dias corridos salvo disposição contratual ou regra processual (CPC arts. 219, 224). Dias não úteis seguem feriados nacionais (Lei 10.607/2002), estaduais e municipais — verifique para cada cancel-by.

Mostra o que está renovando e até quando você precisa cancelar.

## Instructions

1. **Read `~/.claude/plugins/config/claude-for-legal/comercial/renewal-register.yaml`** (the config directory — survives plugin updates).
...
```
/comercial:renewal-tracker --days 180
```

```
/comercial:renewal-tracker --missed
```
...
Lives at `~/.claude/plugins/config/claude-for-legal/comercial/renewal-register.yaml` (the config directory — survives plugin updates). Each entry:
...
- counterparty: "Acme SaaS Inc."
  ...
  notice_method: "email"           # email / portal / AR (carta com aviso de recebimento) / cartório (notificação extrajudicial) / per contract §X
  transit_buffer_days: 0           # 0 para eletrônico/portal, 7 para AR doméstico, 14 para notificação cartorária, ou conforme contrato
...
2. **Roll-back de dia útil conforme lei aplicável.** A lei aplicável do contrato determina quais feriados contam. Brasil: feriados nacionais (Lei 10.607/2002 e legislação anexa), estaduais e municipais da sede da parte notificada — pergunte qual UF/município. Sábado e domingo: rola para sexta-feira anterior. Feriado: rola para o dia útil anterior. **Roll BACK (anterior), nunca posterior** — posterior significa notificação após o fechamento da janela. Se a lei aplicável não for brasileira (incomum em contrato 100% interno; LINDB art. 9º), flague: "Lei aplicável é [X] — roll-back usa feriados nacionais brasileiros como placeholder. Verifique contra calendário de [jurisdição] antes de confiar na data efetiva."
...
**Before proceeding to accept or decline a renewal (including sending a non-renewal notice or letting an auto-renewal run past the cancel-by date):** Read `## Who's using this` in `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md`. If the Role is Non-lawyer:
...
The renewal-watcher agent in this plugin runs this skill on a schedule (weekly by default) and posts the "coming up" report to the channel named in `~/.claude/plugins/config/claude-for-legal/comercial/CLAUDE.md` → `## House style` → where work product goes. Mode 2 is the agent's primary output.
